...
function M.new(semantics_engine)
	...
	local function anchor_ref(v)
		if type(v) == "table" and tostring(v):match("^Lua%.AnchorRef%(") then
			return v
		end
		...
		[C.LocalAssign] = function(n)
			...
			for i = 1, #n.names do
				...
				if n.values[i] then
					...
					if vk == "Number" then
						detail = n.values[i].value
					elseif vk == "String" then
						detail = '"' .. n.values[i].value:sub(1, 20) .. '"'
					elseif vk == "TableCtor" then
						detail = "{...}"
					elseif vk == "FunctionExpr" then
						detail = "function"
					elseif vk == "Call" then
						detail = "call"
					end
				...
				trips[#trips + 1] = { pvm.once(C.DocSymbol(name.value, detail, kind, anchor_ref(name), children)) }
			end
		...
		[C.Assign] = function(n)
			...
			for i = 1, #n.lhs do
				...
				if lv.kind == "LField" then
					...
					trips[#trips + 1] = { pvm.once(C.DocSymbol(lv.key, detail, kind, anchor_ref(lv), {})) }
				end
			end
			if #trips > 0 then
				return pvm.concat_all(trips)
