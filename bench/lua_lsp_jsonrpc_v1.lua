-- bench/lua_lsp_jsonrpc_v1.lua
--
-- JSON-RPC 2.0 stdio loop for lua_lsp_server_core_v1.
--
-- Usage:
--   local Rpc = require("bench.lua_lsp_jsonrpc_v1")
--   Rpc.run_stdio({ parse = ..., position_to_anchor = ..., adapter_opts = ... })

package.path = "./?.lua;./?/init.lua;" .. package.path

local ServerCore = require("bench.lua_lsp_server_core_v1")
...
local function rpc_id_to_lua(id)
    ...
    local k = tostring(id):match("^LuaLsp%.([%w_]+)%(")
...
local function payload_to_lua(core, payload)
    local k = tostring(payload):match("^LuaLsp%.([%w_]+)%(")
...
local function outgoing_to_lua(core, out)
    local k = tostring(out):match("^LuaLsp%.([%w_]+)%(")
...
function M.new(opts)
    ...
    function self:handle_message(msg)
        ...
        local ik = tostring(incoming):match("^LuaLsp%.([%w_]+)%(")
