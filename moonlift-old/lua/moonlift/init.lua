...
local granular_compile
local function_graph = require("moonlift.function_graph")()
local load
local store
local copy
...
local function packed_fn_type(arity)
    ...
    local ty = ffi.typeof("uint64_t (*)(const uint64_t *)")
...
local function make_fast_host_call(addr, arity, param_types, result_t)
    ...
    local null_args = ffi.cast("const uint64_t *", 0)
    return function(...)
        local argc = select("#", ...)
        if argc ~= arity then
            error(("compiled function fast path expects %d arguments, got %d"):format(arity, argc), 2)
        end
        if arity == 0 then
            return unpack_host_result(fp(null_args), result_t)
        end
        local buf = ffi.new("uint64_t[?]", arity)
        for i = 1, arity do
            buf[i - 1] = pack_host_arg(select(i, ...), param_types[i])
        end
        return unpack_host_result(fp(buf), result_t)
    end
end
...
local function for_range_(start, finish, step, body)
    local frame = current_frame()
    if frame == nil then
        error("moonlift.for_range_ may only be used inside moonlift blocks", 2)
    end
    if body == nil then
        body = step
        step = nil
    end
    assert(type(body) == "function", "moonlift.for_range_ body must be a function")
    start = as_expr(start)
    finish = as_expr(finish, start.t)
    if step == nil then
        step = as_expr(1, start.t)
    else
        step = as_expr(step, start.t)
    end
    local lowered_name = fresh_local_name("var")
    local loop_ref = expr({ tag = "local", name = lowered_name }, start.t, make_var_methods(lowered_name, start.t))
    frame.stmts[#frame.stmts + 1] = {
        tag = "for_range",
        name = lowered_name,
        type = lower_type_name(start.t),
        start = start.node,
        finish = finish.node,
        step = step.node,
        inclusive = true,
        scoped = true,
        body = capture_stmt_block(function()
            body(loop_ref)
        end),
    }
    return nil
...
clone_quote_expr = function(node, ctx)
    ...
    elseif tag == "index_addr" then
        return {
            tag = "index_addr",
            base = clone_quote_expr(node.base, ctx),
            index = clone_quote_expr(node.index, ctx),
            elem_size = node.elem_size,
            limit = node.limit and clone_quote_expr(node.limit, ctx) or nil,
            type = node.type,
        }
...
clone_quote_stmt = function(stmt, ctx)
    ...
    elseif tag == "bounds_check" then
        return { tag = "bounds_check", index = clone_quote_expr(stmt.index, ctx), limit = clone_quote_expr(stmt.limit, ctx) }
...
validate_quote_expr = function(node, state)
    ...
    elseif tag == "call" then
        ...
        return
    elseif tag == "index_addr" then
        validate_quote_expr(node.base, state)
        validate_quote_expr(node.index, state)
        if node.limit ~= nil then validate_quote_expr(node.limit, state) end
        return
    elseif tag == "memcmp" then
...
validate_quote_stmt = function(stmt, state)
    ...
    elseif tag == "store" then
        ...
        validate_quote_expr(stmt.value, state)
    elseif tag == "bounds_check" then
        validate_quote_expr(stmt.index, state)
        validate_quote_expr(stmt.limit, state)
    elseif tag == "stack_slot" then
...
local function walk_quote_node_children(node, expr_visitor, stmt_visitor)
    ...
    elseif tag == "load" then
        ...
        return
    elseif tag == "index_addr" then
        walk_quote_expr(node.base, expr_visitor, stmt_visitor)
        walk_quote_expr(node.index, expr_visitor, stmt_visitor)
        if node.limit ~= nil then walk_quote_expr(node.limit, expr_visitor, stmt_visitor) end
        return
    elseif tag == "memcmp" then
...
walk_quote_stmt = function(stmt, expr_visitor, stmt_visitor)
    ...
    elseif tag == "store" then
        ...
        walk_quote_expr(stmt.value, expr_visitor, stmt_visitor)
    elseif tag == "bounds_check" then
        walk_quote_expr(stmt.index, expr_visitor, stmt_visitor)
        walk_quote_expr(stmt.limit, expr_visitor, stmt_visitor)
    elseif tag == "memcpy" or tag == "memmove" then
...
transform_quote_expr = function(node, node_t, expr_mapper, stmt_mapper)
    ...
    elseif tag == "index_addr" then
        out = {
            tag = "index_addr",
            base = select(1, transform_quote_expr(node.base, nil, expr_mapper, stmt_mapper)),
            index = select(1, transform_quote_expr(node.index, nil, expr_mapper, stmt_mapper)),
            elem_size = node.elem_size,
            limit = node.limit and select(1, transform_quote_expr(node.limit, nil, expr_mapper, stmt_mapper)) or nil,
            type = node.type,
        }
    elseif tag == "memcmp" then
        out = {
...
transform_quote_stmt = function(stmt, expr_mapper, stmt_mapper)
    ...
    elseif tag == "bounds_check" then
        out = {
            tag = "bounds_check",
            index = select(1, transform_quote_expr(stmt.index, nil, expr_mapper, stmt_mapper)),
            limit = select(1, transform_quote_expr(stmt.limit, nil, expr_mapper, stmt_mapper)),
        }
    elseif tag == "stack_slot" then
...
QuoteMT.__index = {
    bind = function(self, bindings)
        ...
        local source_text = rawget(self, "__source_text")
        if source_text ~= nil then rawset(out, "__source_text", source_text) end
        local source_env = rawget(self, "__source_host_env")
        if source_env ~= nil then rawset(out, "__source_host_env", source_env) end
        local source_kind = rawget(self, "__source_quote_kind")
        if source_kind ~= nil then rawset(out, "__source_quote_kind", source_kind) end
        return out
    end,
...
local function root_stack_slot_name(node)
    ...
    return root_stack_slot_name(node.value)
        ...
        or root_stack_slot_name(node.base)
        or root_stack_slot_name(node.index)
        or root_stack_slot_name(node.limit)
...
local function root_arg_index(node)
    ...
    return root_arg_index(node.value)
        ...
        or root_arg_index(node.base)
        or root_arg_index(node.index)
        or root_arg_index(node.limit)
...
local function rewrite_layout_param_copies(root, tracked)
    ...
    rewrite_expr = function(node)
        ...
        elseif tag == "memcmp" then
            ...
            if node.addr ~= nil then node.addr = rewrite_expr(node.addr) end
            if node.base ~= nil then node.base = rewrite_expr(node.base) end
            if node.index ~= nil then node.index = rewrite_expr(node.index) end
            if node.limit ~= nil then node.limit = rewrite_expr(node.limit) end
    ...
    rewrite_stmt = function(stmt)
        ...
        elseif stmt.tag == "store" then
            ...
            return stmt
        elseif stmt.tag == "bounds_check" then
            stmt.index = rewrite_expr(stmt.index)
            stmt.limit = rewrite_expr(stmt.limit)
            return stmt
...
local function module(list)
    ...
    local out = {}
    local mod = setmetatable({ funcs = out }, ModuleMT)
    for i = 1, #list do
        local f = list[i]
        assert(is_function(f), "moonlift.module entries must be Moonlift functions")
        out[i] = f
        f.__owner_module = mod
    end
    granular_compile.canonicalize_module(mod)
    return mod
end
...
compile_function = function(f)
    local cached = rawget(f, "__compiled")
    if cached ~= nil then return cached end
    local handle, err
    local source_code = rawget(f, "__source_code") or rawget(f, "__native_source")
    local source_host_env = rawget(f, "__source_host_env")
    if source_code ~= nil then
        assert(type(backend.compile_source_code) == "function", "moonlift rust source frontend is unavailable: backend.compile_source_code is missing")
        handle, err = backend.compile_source_code(source_code, source_host_env)
        assert(
            type(handle) == "number" and handle > 0,
            err or (("moonlift failed to compile rust source function '%s'"):format(f.name))
        )
    else
    ...
    local wrapped = wrap_compiled_handle(f.name, #f.params, handle, function_lowered_params(f), f.result)
    f.__compiled = wrapped
    f.__code_addr = backend.addr(wrapped.handle)
    return wrapped
end
...
granular_compile = require("moonlift.granular_compile") {
    backend = backend,
    wrap_compiled_handle = wrap_compiled_handle,
    function_lowered_params = function_lowered_params,
    compile_function = compile_function,
}

compile_module = function(mod)
    ...
    if source_module ~= nil then
        assert(type(backend.compile_source_module) == "function", "moonlift rust source frontend is unavailable: backend.compile_source_module is missing")
        local handles, err = backend.compile_source_module(source_module, source_host_env)
        assert(type(handles) == "table", err or "moonlift failed to compile rust source module")
        for i = 1, #mod.funcs do
            local f = mod.funcs[i]
    ...
    if module_needs_direct_compile(mod) then
        local lowered = {}
        for i = 1, #mod.funcs do
...
FunctionMT.__call = function(self, ...)
    ...
    local cached = rawget(self, "__compiled")
    if cached ~= nil then return cached end
    local owner_module = rawget(self, "__owner_module")
    if owner_module ~= nil and module_needs_direct_compile(owner_module) then
        return granular_compile.compile_entry(owner_module, self, compile_module)
    end
    if self.lowered ~= nil and lowered_has_direct_call(self.lowered.body) then
        local funcs = function_graph.collect(self)
        local temp_module = setmetatable({ funcs = funcs, __needs_direct_compile = true }, ModuleMT)
        for i = 1, #funcs do funcs[i].__owner_module = temp_module end
        granular_compile.canonicalize_module(temp_module)
        self.__owner_module = temp_module
        return granular_compile.compile_entry(temp_module, self, compile_module)
    end
...
local function is_slice_layout(t)
    return is_struct_type(t) and t.slice_elem ~= nil
end
...
make_field_load = function(base_expr, field_entry)
    ...
    local out = expr({ tag = "load", addr = addr, type = lower_type_name(scalar_t) }, scalar_t)
    local base_ptr_t = base_expr.t
    local base_layout = is_ptr_type(base_ptr_t) and base_ptr_t.elem or nil
    if is_slice_layout(base_layout) and field_entry.name == "ptr" then
        local len_field = base_layout.field_lookup.len
        if len_field ~= nil then
            rawset(out, "__index_limit_expr", make_field_load(base_expr, len_field))
        end
    end
    return out
end
...
make_array_elem_load = function(base_expr, elem_t, index_expr, limit_expr)
    index_expr = as_expr(index_expr)
    local limit_node = nil
    if limit_expr ~= nil then
        limit_node = as_expr(limit_expr).node
    else
        local inherited = rawget(base_expr, "__index_limit_expr")
        if inherited ~= nil then
            limit_node = as_expr(inherited).node
        end
    end
    local addr = {
        tag = "index_addr",
        ...
        elem_size = type_size(elem_t),
        type = "ptr",
    }
    if limit_node ~= nil then addr.limit = limit_node end
...
make_array_elem_store = function(base_expr, elem_t, index_expr, value, limit_expr)
    index_expr = as_expr(index_expr)
    local limit_node = nil
    if limit_expr ~= nil then
        limit_node = as_expr(limit_expr).node
...
StructViewMT.__index = function(self, key)
    ...
    if type(key) ~= "string" then
        if is_slice_layout(s) then
            local base = expr(rawget(self, "_addr"), make_ptr(s))
            return make_array_elem_load(make_field_load(base, s.field_lookup.ptr), s.slice_elem, key, make_field_load(base, s.field_lookup.len))
        end
        return nil
    end
...
StructViewMT.__newindex = function(self, key, value)
    ...
    if type(key) ~= "string" then
        if is_slice_layout(s) then
            local base = expr(rawget(self, "_addr"), make_ptr(s))
            make_array_elem_store(make_field_load(base, s.field_lookup.ptr), s.slice_elem, key, value, make_field_load(base, s.field_lookup.len))
            return
        end
        rawset(self, key, value)
        return
...
ArrayViewMT.__index = function(self, key)
    ...
    return make_array_elem_load(base, a.elem, key, types.usize(a.count))
end
...
ArrayViewMT.__newindex = function(self, key, value)
    ...
    local a = rawget(self, "_array")
    local base = expr(rawget(self, "_addr"), make_ptr(a.elem))
    make_array_elem_store(base, a.elem, key, value, types.usize(a.count))
end
...
ExprMT.__index = function(self, key)
    if is_ptr_type(self.t) and self.t.elem ~= nil then
        ...
        if is_slice_layout(elem) then
            if type(key) == "number" or is_expr(key) then
                return make_array_elem_load(make_field_load(self, elem.field_lookup.ptr), elem.slice_elem, key, make_field_load(self, elem.field_lookup.len))
            end
        end
...
ExprMT.__newindex = function(self, key, value)
    if is_ptr_type(self.t) and self.t.elem ~= nil then
        ...
        if is_slice_layout(elem) and (type(key) == "number" or is_expr(key)) then
            make_array_elem_store(make_field_load(self, elem.field_lookup.ptr), elem.slice_elem, key, value, make_field_load(self, elem.field_lookup.len))
            return
        end
        if type(key) == "number" or is_expr(key) then
...
source_frontend = require("moonlift.source_frontend") {
    backend = backend,
    new_type = new_type,
    types = types,
    is_expr = is_expr,
    ...
    switch_ = switch_,
    while_ = while_,
    for_range_ = for_range_,
    break_ = break_,
    continue_ = continue_,
    is_ir_quote = is_ir_quote,
    quote_expr_capture = QuoteNS.expr,
    quote_block_capture = QuoteNS.block,
    quote_stmt_capture = QuoteNS.stmt,
...
local QuoteAPI = setmetatable({}, {
    __index = QuoteNS,
    __call = function(_, spec, host_env)
        if type(spec) == "string" then
            if host_env ~= nil then assert(type(host_env) == "table", "moonlift.quote host_env must be a table or nil") end
            return source_frontend.quote.block(spec, host_env or source_frontend.caller_env(1))
        end
        return QuoteNS.block(spec)
    end,
})

QuoteAPI.expr = function(spec, host_env)
    if type(spec) == "string" then
        if host_env ~= nil then assert(type(host_env) == "table", "moonlift.quote.expr host_env must be a table or nil") end
        return source_frontend.quote.expr(spec, host_env or source_frontend.caller_env(1))
    end
    return QuoteNS.expr(spec)
end

QuoteAPI.stmt = function(spec, host_env)
    if type(spec) == "string" then
        if host_env ~= nil then assert(type(host_env) == "table", "moonlift.quote.stmt host_env must be a table or nil") end
        return source_frontend.quote.stmt(spec, host_env or source_frontend.caller_env(1))
    end
    return QuoteNS.stmt(spec)
end

QuoteAPI.block = function(spec, host_env)
    if type(spec) == "string" then
        if host_env ~= nil then assert(type(host_env) == "table", "moonlift.quote.block host_env must be a table or nil") end
        return source_frontend.quote.block(spec, host_env or source_frontend.caller_env(1))
    end
    return QuoteNS.block(spec)
end

QuoteAPI.func = function(source, host_env)
    if host_env ~= nil then assert(type(host_env) == "table", "moonlift.quote.func host_env must be a table or nil") end
    return source_frontend.quote.func(source, host_env or source_frontend.caller_env(1))
end

QuoteAPI.type = function(source, host_env)
    if host_env ~= nil then assert(type(host_env) == "table", "moonlift.quote.type host_env must be a table or nil") end
    return source_frontend.quote.type(source, host_env or source_frontend.caller_env(1))
end

QuoteAPI.module = function(source, host_env)
    if host_env ~= nil then assert(type(host_env) == "table", "moonlift.quote.module host_env must be a table or nil") end
    return source_frontend.quote.module(source, host_env or source_frontend.caller_env(1))
end
...
M = {
    ...
    sext = sext,
    bitcast = bitcast,
    code = function(source, host_env)
        if host_env ~= nil then assert(type(host_env) == "table", "moonlift.code host_env must be a table or nil") end
        return source_frontend.code(source, host_env or source_frontend.caller_env(1))
    end,
    expr = function(source, host_env)
        if host_env ~= nil then assert(type(host_env) == "table", "moonlift.expr host_env must be a table or nil") end
        return source_frontend.expr(source, host_env or source_frontend.caller_env(1))
    end,
    type = function(source, host_env)
        if host_env ~= nil then assert(type(host_env) == "table", "moonlift.type host_env must be a table or nil") end
        return source_frontend.type(source, host_env or source_frontend.caller_env(1))
    end,
    asdl = function(source)
        return asdl_frontend.compile(source, asdl_frontend.caller_env(1))
    end,
    func = func,
    extern = function(x, host_env)
        if type(x) == "string" and source_frontend.looks_like_source_snippet(x) then
            if host_env ~= nil then assert(type(host_env) == "table", "moonlift.extern host_env must be a table or nil") end
            return source_frontend.extern(x, host_env or source_frontend.caller_env(1))
        end
        return extern(x)
    end,
    import_module = import_module,
    invoke = invoke,
    module = function(x, host_env)
        if type(x) == "string" then
            if host_env ~= nil then assert(type(host_env) == "table", "moonlift.module host_env must be a table or nil") end
            return source_frontend.module(x, host_env or source_frontend.caller_env(1))
        end
        return module(x)
    end,
    if_ = if_,
    ...
    call4 = backend.call4,
    stats = backend.stats,
    exact_tostring = function(value)
        local ffi = get_ffi()
        if ffi ~= nil and type(value) == "cdata" then
    ...
    debug = require("moonlift.debug") { backend = backend, code = source_frontend.code },
    parse = source_frontend.parse,
