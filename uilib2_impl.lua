-- uilib2_impl.lua
--
-- Implementation for uilib2: UI library with design system and pointer-variant paint packs.
--
-- Key change from uilib: leaf colors are DS.ColorPack (4 pointer-phase variants),
-- resolved at compile time by the DS boundary. The paint loop selects one variant
-- per command based on the current pointer phase — no rule matching, no token lookup.

local pvm = require("pvm")
local schema = require("uilib2_asdl")
local T = schema.T
...
-- View (Kind singletons + constructors in table to save locals)
...
local K_DYNTEXT, K_DYNMETER, K_DYNPLAYHEAD = T.View.DynText, T.View.DynMeter, T.View.DynPlayhead
...
local resolve_style = pvm.lower("ds.resolve_style", resolve_style_impl)
...
local stats = {
    measure = { name = "uilib2.measure", calls = 0, hits = 0 },
    compile = { name = "uilib2.compile", calls = 0, hits = 0 },
    assemble = { name = "uilib2.assemble", calls = 0, hits = 0 },
}
...
local text_intrinsic_widths, shaped_text, shape_text
do
...
get_font = function(font_id)
    ...
    if love and love.graphics and love.graphics.getFont then return love.graphics.getFont() end
    return nil
end
        ...
        if raw_text_width(font_id, candidate) <= max_w then
            ...
            if raw_text_width(font_id, t) <= max_w then
                ...
                for j = 1, #chars do
                    local c = chars[j]
                    local cand = frag .. c
                    if frag == "" or raw_text_width(font_id, cand) <= max_w then frag = cand
                    else out[#out+1] = frag; frag = c end
                end
                line = frag
            end
        end
...
text_intrinsic_widths = function(style, text)
    ...
    for i = 1, #paragraphs do
        ...
        elseif style.wrap == TXT_WORDWRAP then
            ...
            for j = 1, #words do local w = raw_text_width(style.font_id, words[j]); if w > local_min then local_min = w end end
        else
            local_min = 0
    ...
    if max_w ~= math.huge then
        if style.wrap == TXT_NOWRAP then
            ...
            elseif #lines > 0 and style.overflow == OV_CLIP then
                local s = lines[1] or ""
                while #s > 0 and raw_text_width(style.font_id, s) > max_w do s = string.sub(s, 1, #s-1) end
                lines[1] = s
            end
...
-- DynText/DynMeter/DynPlayhead measure stubs
function T.UI.DynText:_measure_uncached(constraint)
    local inner = box_content_constraint(self.box, constraint)
    local max_w = span_available(inner.w); local max_h = span_available(inner.h)
    local shape = shape_text(self.style, self.sample, max_w, max_h)
    local min_w, max_intr_w = text_intrinsic_widths(self.style, self.sample)
    local raw = { min_w = min_w, min_h = resolve_line_height(self.style.line_height, self.style.font_id), max_w = max_intr_w, max_h = shape.used_h, used_w = shape.used_w, used_h = shape.used_h, baseline = make_baseline(shape.baseline) }
    return apply_box_measure(self.box, constraint, raw)
end
function T.UI.DynMeter:_measure_uncached(constraint) return apply_box_measure(self.box, constraint, raw_measure_zero()) end
function T.UI.DynPlayhead:_measure_uncached(constraint) return apply_box_measure(self.box, constraint, raw_measure_zero()) end
...
function ui.clear_cache()
    ...
    wipe(measure_cache); wipe(compile_cache); wipe(assemble_cache)
    wipe(shape_cache)
...
function ui.report()
    ...
    return table.concat({ line(stats.measure), line(stats.compile), line(stats.assemble), string.format("%-20s calls=%-6d hits=%-6d rate=%d%%", rs.name, rs.calls, rs.hits, (rs.calls > 0) and math.floor((rs.hits / rs.calls) * 100 + 0.5) or 0) }, "\n")
end

require("uilib2_exec")(ui, T, {
    ...
    LH_AUTO = LH_AUTO, INF = INF, BASELINE_NONE = BASELINE_NONE,
    K_RECT = K_RECT, K_TEXTBLOCK = K_TEXTBLOCK, K_PUSH_CLIP = K_PUSH_CLIP,
    K_POP_CLIP = K_POP_CLIP, K_PUSH_TX = K_PUSH_TX, K_POP_TX = K_POP_TX,
    K_DYNTEXT = K_DYNTEXT, K_DYNMETER = K_DYNMETER, K_DYNPLAYHEAD = K_DYNPLAYHEAD,
...
}, stats, measure_cache, compile_cache, assemble_cache)
