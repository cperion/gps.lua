...
impl<'a> Parser<'a> {
    ...
    fn parse_scalar_type_path(&mut self) -> Option<Path> {
        ...
        if matches!(
            ...
            "void" | "bool" | "i8" | "i16" | "i32" | "i64" | "u8" | "u16" | "u32"
                | "u64" | "isize" | "usize" | "index" | "f32" | "f64" | "byte"
        ) {
            let span = self.current_span();
            self.bump();
            Some(Path {
    ...
    fn parse_loop_var_init_after_name(&mut self, name: String, start: usize) -> Result<LoopVarInit, ParseError> {
        self.expect_simple(TokenKind::Colon)?;
        let ty = self.parse_type_expr()?;
        self.expect_simple(TokenKind::Assign)?;
    ...
    fn parse_loop_var_init(&mut self) -> Result<LoopVarInit, ParseError> {
        let (name, span) = self.expect_ident()?;
        self.parse_loop_var_init_after_name(name, span.start)
    }

    fn parse_loop_head(&mut self) -> Result<LoopHead, ParseError> {
        let (name, span) = self.expect_ident()?;
        if self.eat_simple(TokenKind::KwOver) {
            let domain = self.parse_expr()?;
            let mut carries = Vec::new();
            while self.eat_simple(TokenKind::Comma) {
                carries.push(self.parse_loop_var_init()?);
            }
            let end = carries
                .last()
                .map(|v| v.span.end)
                .unwrap_or(domain.span.end);
            Ok(LoopHead::Over {
                index: name,
            ...
            let mut vars = vec![self.parse_loop_var_init_after_name(name, span.start)?];
            while self.eat_simple(TokenKind::Comma) {
                if self.check_simple(&TokenKind::KwWhile) {
                    break;
                }
                vars.push(self.parse_loop_var_init()?);
            }
            self.expect_simple(TokenKind::KwWhile)?;
            let cond = self.parse_expr()?;
            Ok(LoopHead::While {
                vars,
                span: Span::new(span.start, cond.span.end),
                cond: Box::new(cond),
            })
        }
    }

    fn parse_loop_next_section(&mut self) -> Result<Vec<LoopNextAssign>, ParseError> {
        self.expect_simple(TokenKind::KwNext)?;
        self.skip_separators();
        let mut out = Vec::new();
        while !self.check_simple(&TokenKind::KwEnd) && !self.check_simple(&TokenKind::Eof) {
            let (name, span) = self.expect_ident()?;
    ...
    fn parse_loop_stmt(&mut self) -> Result<Stmt, ParseError> {
        let start = self.expect_simple(TokenKind::KwLoop)?.span.start;
        let head = self.parse_loop_head()?;
        let body = self.parse_stmt_block_until(|k| matches!(k, TokenKind::KwNext | TokenKind::KwEnd))?;
        let next = if self.check_simple(&TokenKind::KwNext) {
            self.parse_loop_next_section()?
        } else {
            Vec::new()
        };
        self.expect_simple(TokenKind::KwEnd)?;
...
mod tests {
    ...
    #[test]
    fn parse_loop_stmt_and_expr() {
        let item = parse_code(
            "func sum(xs: &Pair) -> i32\n    loop i: index = 0, acc: i32 = 0 while i < 4\n        let xi: i32 = 1\n    next\n        acc = acc + xi\n        i = i + 1\n    end\n\n    return loop j over range(4), total: i32 = 0\n    next\n        total = total + j\n    end -> total\nend\n",
        )
        .unwrap();
        let ItemKind::Func(func) = item.kind else {
            panic!("expected func item");
        };
        assert!(matches!(func.body.stmts[0], Stmt::Loop(_)));
        match &func.body.stmts[1] {
            Stmt::Return { value: Some(expr), .. } => match &expr.kind {
                ExprKind::Loop(_) => {}
                _ => panic!("expected loop expr"),
            },
            _ => panic!("expected return loop expr"),
        }
    }
}
