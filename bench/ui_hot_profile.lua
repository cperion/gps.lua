...
local iters = tonumber(arg[3]) or 2000
local stage = arg[4] or "full"
local mode = arg[5] or "vf2i1m1"
local outfile = arg[6] or ("./bench/ui_hot_" .. workload .. "_" .. stage .. ".out")

if not lib.workloads[workload] then
    io.stderr:write("usage: luajit bench/ui_hot_profile.lua [flat_list|text_heavy|nested_panels] [n] [iters] [full|lower|render] [jit.p mode] [outfile]\n")
    os.exit(1)
end
...
lib.reset_phases()
run_once() -- warm once before profiling
...
print(string.format("wrote hot profile to %s  workload=%s n=%d iters=%d stage=%s", outfile, workload, n, iters, stage))
