...
local function resolve_axis(axis)
    if axis == S.Row then return Layout.LRow end
    return Layout.LCol
end

local function resolve_wrap(wrap)
    if wrap == S.WrapOn then return Layout.LWrap end
    return Layout.LNoWrap
end
...
local function resolve_sizing(length)
    ...
    if length == S.Auto then return Layout.SAuto end
    if length == S.Hug then return Layout.SHug end
    if length == S.Fill then return Layout.SFill end
    if cls == S.Fixed then return Layout.SFixed(length.px) end
    if cls == S.Frac then return Layout.SFrac(fraction_value(length.value)) end
    error("ui.resolve: unknown sizing", 2)
end

local function resolve_basis(basis)
    local cls = pvm.classof(basis)
    if basis == S.BasisAuto then return Layout.BasisAuto end
    if basis == S.BasisHug then return Layout.BasisHug end
    if cls == S.BasisFixed then return Layout.BasisFixed(basis.px) end
    if cls == S.BasisFrac then return Layout.BasisFrac(fraction_value(basis.value)) end
    error("ui.resolve: unknown basis", 2)
end

local function resolve_min(length)
    local cls = pvm.classof(length)
    if length == S.Auto or length == S.Hug then return Layout.NoMin end
    if length == S.Fill then return Layout.MinFrac(1) end
    if cls == S.Fixed then return Layout.MinPx(length.px) end
    if cls == S.Frac then return Layout.MinFrac(fraction_value(length.value)) end
    error("ui.resolve: unknown min", 2)
end

local function resolve_max(length)
    local cls = pvm.classof(length)
    if length == S.Auto or length == S.Hug then return Layout.NoMax end
    if length == S.Fill then return Layout.MaxFrac(1) end
    if cls == S.Fixed then return Layout.MaxPx(length.px) end
    if cls == S.Frac then return Layout.MaxFrac(fraction_value(length.value)) end
...
local function resolve_margin_value(value, theme)
    ...
    if value == S.MAuto then
        return Layout.MarginAuto
    end
    if cls == S.MSpace then
...
local function resolve_track(track)
    ...
    if track == S.TrackAuto then return Layout.TrackAuto end
    if cls == S.TrackFr then return Layout.TrackFr(track.fr) end
    if cls == S.TrackFixed then return Layout.TrackFixed(track.px) end
    if cls == S.TrackMinMax then return Layout.TrackMinMax(track.min_px, track.max_px) end
...
local resolve = pvm.lower("ui.resolve", resolve_impl)
