...
### 8.6 Canonical loop statement

```text
loop_stmt       ::= "loop" loop_head loop_body [ loop_next ] "end"

loop_head       ::= loop_while_head | loop_over_head

loop_while_head ::= loop_var_init { "," loop_var_init } "while" expr
loop_over_head  ::= ident "over" expr { "," loop_var_init }

loop_var_init   ::= ident ":" type_expr "=" expr

loop_body       ::= newline { stmt_sep stmt }

loop_next       ::= "next" newline
                    loop_next_assign { stmt_sep loop_next_assign }
...
- `loop ... while ...` is the explicit phi-loop / state-loop form.
- `loop ... over ...` is the domain loop form and the preferred counted-loop spelling.
- In the current implementation, `range(...)` and `zip_eq(...)` are recognized domain expressions.
- `next` assignments are simultaneous and update loop state at the latch.
- Loop-variable source order is not intended as a performance contract; the compiler canonicalizes a primary induction variable when it can prove one.
- For branchy next-state updates, the preferred source pattern is to compute `next_*` temporaries in the loop body and then assign those temporaries in `next`.
...
### 11.17 Canonical loop expressions

```text
loop_expr       ::= "loop" loop_head loop_body [ loop_next ] "end" "->" expr
```

The `loop_head` and `loop_next` productions are shared with `loop_stmt`.

### 11.18 Anonymous function expressions
