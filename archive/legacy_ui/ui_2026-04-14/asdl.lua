...
T:Define [[
    module Layout {
        ...
        Track = TrackAuto
              | TrackPx(number px) unique
              | TrackFr(number weight) unique
...
--
-- This is the library-facing tree. It carries identity, interaction nodes,
-- style-scope application, and authored layout.
--
-- StyleScope(name, flags, child) is a semantic style scope. Lowering threads
-- the resolved style down the subtree and may also emit concrete panel chrome.
-- ─────────────────────────────────────────────────────────────
...
T:Define [[
    module SemUI {
        ...
        Node = Empty
             ...
             | Interact(string key_id,
                        string hit_id,
                        string press_id,
                        string focus_id,
                        SemUI.Node child) unique

             | StyleScope(string name,
                          DS.Flag* flags,
                          SemUI.Node child) unique
             ...
             | Grid(Layout.Track* cols,
                    Layout.Track* rows,
                    number gap_x,
                    number gap_y,
                    Layout.Box box,
                    SemUI.GridItem* children) unique
...
--
-- This is the reducer-facing tree. Design-system resolution has already
-- happened. Interaction identity remains explicit via UI.Interact.
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
-- SolvedUI — layout-solved reducer input
--
-- This is the next architectural layer: UI plus resolved frames/extents. Draw
-- and hit should eventually consume this instead of re-solving layout on every
-- call. For now it exists as the typed target for that boundary.
-- ─────────────────────────────────────────────────────────────

T:Define [[
    module SolvedUI {
        FlexItem = (SolvedUI.Node node)
        GridItem = (SolvedUI.Node node)
        ...
        Node = Empty
             ...
             | Flex(Facts.Frame frame,
                    SolvedUI.FlexItem* children)

             | Grid(Facts.Frame frame,
                    SolvedUI.GridItem* children)

             | Pad(Facts.Frame frame,
                   SolvedUI.Node child)

             | Stack(Facts.Frame frame,
                     SolvedUI.Node* children)

             | Clip(Facts.Frame frame,
                    SolvedUI.Node child)

             | Transform(Facts.Frame frame,
                         number tx,
                         number ty,
                         SolvedUI.Node child)
             ...
             | Overlay(SolvedUI.Node base,
                       Paint.Node overlay)

             | Spacer(Facts.Frame frame)
    }

    module Solve {
        LayoutRequest = (UI.Node node,
                         Facts.Frame viewport)
    }
]]
