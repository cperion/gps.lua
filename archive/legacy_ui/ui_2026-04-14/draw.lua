...
-- nodes, and emits drawing calls to a backend.
--
...
local mtColorPack = classof(T.DS.ColorPack(0, 0, 0, 0))
...
local function resolve_paint_scalar(_, scalar)
    return scalar.n
end

local function resolve_paint_text(_, text_value)
    return text_value.text
end

local function resolve_paint_color(_, color_value, active_id, tag, opts)
...
local draw_solved_node

local function interact_draw_id(node, active_id)
    if node.press_id and node.press_id ~= "" then
        return resolve_draw_id(node.press_id, active_id)
    end
    if node.hit_id and node.hit_id ~= "" then
        return resolve_draw_id(node.hit_id, active_id)
    end
    if node.focus_id and node.focus_id ~= "" then
        return resolve_draw_id(node.focus_id, active_id)
    end
    if node.key_id and node.key_id ~= "" then
        return resolve_draw_id(node.key_id, active_id)
    end
    return active_id
end
...
local function draw_text_node(node, frame, active_id, ctx)
    ...
    if node.kind == "RuntimeText" and ctx.opts.resolve_runtime_text then
        text = ctx.opts.resolve_runtime_text(node, ctx.opts) or text
    end
    if clipped then
        push_clip(ctx, x, y, frame.w, frame.h)
    end
    draw_text_payload(text, x, y, frame.w, frame.h, node.style, resolve_draw_id(active_id, node.tag), ctx)
    if clipped then
        pop_clip(ctx)
    end
end
...
draw_solved_node = function(node, active_id, ctx)
    if not node then
        return
    end
    ...
    elseif kind == "Interact" then
        return draw_solved_node(node.child, interact_draw_id(node, active_id), ctx)
    elseif kind == "Flex" then
        for i = 1, #node.children do
            draw_solved_node(node.children[i].node, active_id, ctx)
        end
        return
    elseif kind == "Grid" then
        ...
        return
    elseif kind == "Pad" or kind == "Stack" then
        if kind == "Pad" then
            return draw_solved_node(node.child, active_id, ctx)
        end
        for i = 1, #node.children do
            draw_solved_node(node.children[i], active_id, ctx)
        end
        return
    elseif kind == "Clip" then
        push_clip(ctx, frame.x + ctx.tx, frame.y + ctx.ty, frame.w, frame.h)
        draw_solved_node(node.child, active_id, ctx)
        pop_clip(ctx)
        return
    ...
    elseif kind == "ScrollArea" then
        ...
        draw_solved_node(node.child, resolve_draw_id(node.id, active_id), ctx)
        pop_clip(ctx)
        return
    ...
    elseif kind == "Rect" then
        draw_box_chrome(frame, node.style, active_id, node.tag, ctx)
        return
    elseif kind == "Text" or kind == "RuntimeText" then
        draw_text_node(node, frame, resolve_draw_id(active_id, node.tag), ctx)
        return
    elseif kind == "CustomPaint" then
        if ctx.opts.draw_custom_paint and ctx.opts.draw_custom_paint(node, resolve_draw_id(active_id, node.tag), ctx, {
            backend_set_color = backend_set_color,
            backend_set_font = backend_set_font,
            fill_rect = fill_rect,
            stroke_rect = stroke_rect,
            draw_text_payload = draw_text_payload,
            push_clip = push_clip,
            pop_clip = pop_clip,
            visible_in_clip = visible_in_clip,
        }) then
            return
        end
        local paint = node.paint
        if ctx.opts.resolve_custom_paint then
            paint = ctx.opts.resolve_custom_paint(node, ctx.opts) or paint
        end
        push_offset(ctx, frame.x, frame.y)
        draw_paint_node(paint, resolve_draw_id(active_id, node.tag), ctx)
        pop_offset(ctx)
        return
    elseif kind == "Overlay" then
        draw_solved_node(node.base, active_id, ctx)
        push_offset(ctx, node.base.frame.x, node.base.frame.y)
    ...
    error("ui.draw: unsupported SolvedUI node kind " .. tostring(kind), 2)
end

function draw.draw_solved(node, frame, opts)
    local ctx = build_ctx(frame, opts)
    draw_solved_node(node, nil, ctx)
end

function draw.draw(node, frame, opts)
    return draw.draw_solved(solve.node(node, frame, opts), frame, opts)
end
