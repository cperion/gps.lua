...
local function leaf_intrinsic(node, constraint, text_system)
    ...
    if node.text ~= nil then
        local tl = text_layout_one(node.text, Layout.Constraint(inner_w, HUGE), text_system)
...
local function measure_leaf(self, constraint, text_system)
    local box = self.box
    local intrinsic_w, intrinsic_h, baseline = leaf_intrinsic(self, constraint, text_system)
...
local function measure_flow(self, constraint, text_system)
    local box = self.box
    local pad = box.padding
    local pad_h = pad.left + pad.right
    local pad_v = pad.top + pad.bottom
...
local function measure_flex(self, constraint, text_system)
    local box = self.box
    local pad = box.padding
    local pad_h = pad.left + pad.right
    local pad_v = pad.top + pad.bottom
...
local function measure_grid(self, constraint, text_system)
    local box = self.box
    local pad = box.padding
    local pad_h = pad.left + pad.right
    local pad_v = pad.top + pad.bottom
...
local function measure_scroll(self, constraint, text_system)
    local box = self.box
    local pad = box.padding
    local pad_h = pad.left + pad.right
    local pad_v = pad.top + pad.bottom
    local inner_w = finite(constraint.max_w) and max0(constraint.max_w - pad_h) or HUGE
    local inner_h = finite(constraint.max_h) and max0(constraint.max_h - pad_v) or HUGE
    local child = measure_one(self.child, scroll_child_constraint(self.axis, inner_w, inner_h), text_system)
...
measure_phase = pvm.phase("ui.measure", {
    [Layout.Leaf] = function(self, constraint, text_system)
        return measure_leaf(self, constraint, text_system)
    end,

    [Layout.Flow] = function(self, constraint, text_system)
        return measure_flow(self, constraint, text_system)
    end,

    [Layout.Paint] = function(self, constraint, text_system)
        return measure_paint(self, constraint, text_system)
    end,

    [Layout.Scroll] = function(self, constraint, text_system)
        return measure_scroll(self, constraint, text_system)
    end,

    [Layout.Flex] = function(self, constraint, text_system)
        return measure_flex(self, constraint, text_system)
    end,

    [Layout.Grid] = function(self, constraint, text_system)
        return measure_grid(self, constraint, text_system)
    end,

    [Layout.WithInput] = function(self, constraint, text_system)
        return pvm.once(pvm.one(measure_phase(self.child, constraint, text_system)))
    end,
})

function M.root(node, max_w, max_h, text_system)
    return measure_phase(node, Layout.Constraint(max_w, max_h), text_system)
end
