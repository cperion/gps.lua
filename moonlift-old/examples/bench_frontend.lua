...
local HOT_SOURCE_META = backend.source_meta_code(HOT_SOURCE_TEXT)
local HOT_SOURCE_FN = code(HOT_SOURCE_TEXT)
...
local HOT_MODULE_META = backend.source_meta_module(HOT_MODULE_TEXT)
local HOT_SOURCE_MODULE = module(HOT_MODULE_TEXT)
...
local function bench_function_build_split()
    ...
    local t_native_meta_hot, native_meta = timeit(BUILD_ITERS, function()
        return backend.source_meta_code(HOT_SOURCE_TEXT)
    end)
    ...
    print_avg("native meta hot", "us", t_native_meta_hot, BUILD_ITERS, native_meta.name)
...
local function bench_module_compile_split()
    ...
    local t_source_meta_hot, source_mod_meta = timeit(COMPILE_ITERS, function()
        return backend.source_meta_module(HOT_MODULE_TEXT)
    end)
    ...
    assert(source_mod_ast ~= nil and source_mod_meta ~= nil)
    ...
    print_avg("module native meta", "us", t_source_meta_hot, COMPILE_ITERS, #source_mod_meta.funcs)
