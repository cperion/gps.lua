...
local function build_geom_phase(rt, name)
    return iwj.phase(rt, name, {
...
local rt = iwj.runtime { result_capacity = 16 * 1024 * 1024 }
local phase = build_geom_phase(rt, "geom_iwatjit")
...
add_row("phase_cold(mix)", bench_ms(5, function()
    local fresh = iwj.runtime { result_capacity = 16 * 1024 * 1024 }
    local cold = build_geom_phase(fresh, "geom_iwatjit_cold")
    return phase_sum(mix, cold)
end), N_MIX / 1e3)
phase_sum(mix, phase)
add_row("phase_hot(mix)", bench_ms(20, function() return phase_sum(mix, phase) end), N_MIX / 1e3)

print("iwatjit handle bench")
print("ported from bench/pvm_handle_storage_bench.lua")
...
print()
print("iwatjit report:")
print(iwj.report_string(rt))
local mem = iwj.memory_stats(rt)
print(string.format("result used=%d cap=%s live_slots=%d evictions=%d",
    mem.result.used,
    tostring(mem.result.cap),
    mem.result.live_slots,
    mem.result.evictions))
