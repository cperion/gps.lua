...
local iters = tonumber(arg[3]) or 30
local stage = arg[4] or "full"
local mode = arg[5] or "tbis"
local outfile = arg[6] or ("./bench/ui_cold_" .. workload .. "_" .. stage .. ".trace")

if not lib.workloads[workload] then
    io.stderr:write("usage: luajit bench/ui_cold_trace.lua [flat_list|text_heavy|nested_panels] [n] [iters] [full|lower|render] [jit.dump mode] [outfile]\n")
    os.exit(1)
end
...
print(string.format("wrote cold trace to %s  workload=%s n=%d iters=%d stage=%s", outfile, workload, n, iters, stage))
