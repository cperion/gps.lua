...
`while` is the irregular escape hatch loop.
It permits `break`, `continue`, and arbitrary control flow, but it is not the canonical fast-loop form.

### 10.6 Canonical `loop ... while ...`
...
This is the explicit phi-loop form.

Semantic rules:

- loop variables are declared in the header
- loop variables are read-only in the body
- loop state updates happen only in the `next` section
- `next` assignments are simultaneous
- source order of loop variables is semantic, but not performance-significant: the compiler identifies and canonicalizes a primary induction variable when the loop has counted-loop shape
- `end -> expr` evaluates in the loop exit environment
- the implementation lowers canonical `loop ... while ...` to an explicit loop-state IR node instead of first desugaring it into ordinary source-level mutable-local syntax
- `loop ... while ...` is the generic phi/state loop; when the loop is really a counted/domain traversal, `loop ... over ...` is the preferred canonical form
- for branchy carried-state updates, prefer computing explicit body-local `next_*` temporaries and then assigning them in `next`, rather than building a large nested value-expression tree inside one `next` assignment

### 10.7 Canonical `loop ... over ...`

```text
loop i over zip_eq(dst, src), y: f32 = z1
    let yn = a * y + b * src[i]
    dst[i] = yn
next
    y = yn
...
This is the domain-loop form and the preferred counted/memory-kernel loop.

Semantic rules:

- `i` is an implicit read-only index variable
- the domain supplies the trip count and bounds proof source
- carried state appears after the domain and is updated through `next`
- in v1, the implemented domain forms are:
  - `range(stop)`
  - `range(start, stop)`
  - `zip_eq(a, b, ...)`
  - a single bounded array/slice value such as `xs`
- `loop ... over ...` is the preferred fast counted/memory-kernel form because it preserves the iteration domain explicitly for lowering
- branchy carried-state updates in a domain loop follow the same rule as `loop ... while ...`: compute explicit `next_*` temporaries in the body, then commit them in `next`
