return function(ctx)
    ...
    local for_range_ = assert(ctx.for_range_)
    ...
    local source_quote_helpers
    local shallow_copy

    local function source_merge_host_bindings(base_env, bindings)
        if bindings == nil or next(bindings) == nil then
            return base_env or _G
        end
        return setmetatable(shallow_copy(bindings), { __index = base_env or _G })
    end
    ...
    local function source_host_float_literal(n)
        local s = string.format("%.17g", n)
        if not s:find("[%.eE]", 1) then s = s .. ".0" end
        return s
    end

    local function source_host_expr_literal(value)
        if is_expr(value) then
            local node = value.node or {}
            if node.tag == "bool" then return node.value and "true" or "false" end
            if node.tag == "f32" or node.tag == "f64" then
                return source_host_float_literal(assert(tonumber(node.value), "moonlift host splice float constant is invalid"))
            end
            if node.tag == "i8" or node.tag == "i16" or node.tag == "i32" or node.tag == "i64"
                or node.tag == "u8" or node.tag == "u16" or node.tag == "u32" or node.tag == "u64"
                or node.tag == "ptr" then
                return tostring(assert(node.value, "moonlift host splice integer constant is invalid"))
            end
            error("moonlift rust-only source splice currently supports only literal Moonlift expressions", 2)
        end
        if value == nil then return "nil" end
        if type(value) == "boolean" then return value and "true" or "false" end
        if type(value) == "number" then
            if value == math.floor(value) then return tostring(value) end
            return source_host_float_literal(value)
        end
        if type(value) == "string" then return string.format("%q", value) end
        error("moonlift rust-only source splice currently supports only Lua scalar literals or literal Moonlift expressions", 2)
    end

    local function source_host_hole_literal(value, host_env)
        if type(value) == "string" then return value end
        if is_expr(value) then
            local src = rawget(value, "__source_expr")
            local src_env = rawget(value, "__source_host_env")
            if type(src) == "string" and (src_env == nil or src_env == host_env) then return src end
            local node = value.node or {}
            if node.tag == "bool" then return node.value and "true" or "false" end
            if node.tag == "f32" or node.tag == "f64" then
                return source_host_float_literal(assert(tonumber(node.value), "moonlift host hole float constant is invalid"))
            end
            if node.tag == "i8" or node.tag == "i16" or node.tag == "i32" or node.tag == "i64"
                or node.tag == "u8" or node.tag == "u16" or node.tag == "u32" or node.tag == "u64"
                or node.tag == "ptr" then
                return tostring(assert(node.value, "moonlift host hole integer constant is invalid"))
            end
            error("moonlift rust-only source hole currently supports bound source exprs or literal Moonlift expressions", 2)
        end
        if value == nil or type(value) == "boolean" or type(value) == "number" then
            return source_host_expr_literal(value)
        end
        error("moonlift rust-only source hole currently supports expression source strings, source expr values, or Lua scalar literals", 2)
    end

    local function source_host_type_literal(value)
        if type(value) == "string" then return value end
        assert(type(value) == "table", "moonlift rust-only type splice expects a Moonlift type value or type source string")
        if value.family == "ptr" then
            return "&" .. source_host_type_literal(value.elem or types.byte)
        end
        if value.layout_kind == "array" then
            return ("[%d]%s"):format(value.count, source_host_type_literal(value.elem))
        end
        if value.slice_elem ~= nil then
            return "[]" .. source_host_type_literal(value.slice_elem)
        end
        if value.family == "funcptr" then
            local parts = {}
            for i = 1, #(value.params or {}) do
                parts[i] = source_host_type_literal(value.params[i].t or value.params[i])
            end
            local out = "func(" .. table.concat(parts, ", ") .. ")"
            if value.result ~= nil and value.result ~= types.void then
                out = out .. " -> " .. source_host_type_literal(value.result)
            end
            return out
        end
        assert(type(value.name) == "string", "moonlift rust-only type splice expects a named Moonlift type")
        return value.name
    end

    local function source_host_item_literal(value)
        if type(value) == "string" then return value end
        if type(value) == "table" and getmetatable(value) == FunctionMT then
            local src = rawget(value, "__source_code") or rawget(value, "__native_source")
            assert(type(src) == "string", "moonlift rust-only item splice expects a source-origin function or source text")
            local src_env = rawget(value, "__source_host_env")
            if src_env ~= nil then return source_host_splice_bundle(src, src_env) end
            return src
        end
        if type(value) == "table" and rawget(value, "funcs") ~= nil then
            local src = rawget(value, "__source_module") or rawget(value, "__native_source")
            assert(type(src) == "string", "moonlift rust-only item splice expects a source-origin module or source text")
            local src_env = rawget(value, "__source_host_env")
            if src_env ~= nil then return source_host_splice_bundle(src, src_env) end
            return src
        end
        error("moonlift rust-only item splice expects source text or a source-origin function/module", 2)
    end

    rawset(_G, "__moonlift_eval_source_splice", function(host_env, source, context)
        local env = host_env or _G
        if context == "hole" then
            local value = env[source]
            assert(value ~= nil, ("moonlift rust-only source hole '%s' is unbound"):format(tostring(source)))
            return source_host_hole_literal(value, env)
        end
        local value = eval_host_source(source, env, "=(moonlift.host.splice." .. tostring(context) .. ")")
        local bundled = source_quote_helpers and source_quote_helpers.maybe_splice_value(value, context, env)
        if bundled ~= nil then return bundled end
        if context == "expr" then return source_host_expr_literal(value) end
        if context == "type" then return source_host_type_literal(value) end
        if context == "item" then return source_host_item_literal(value) end
        error("unknown moonlift host splice context: " .. tostring(context), 2)
    end)
    ...
    local native_type_meta_cache = {}
    local native_extern_meta_cache = {}
    local source_native_type_from_meta
    local make_source_func_type
    ...
    local function source_expr_result_type_by_name(name)
        if name == "ptr" then return make_ptr(types.byte) end
        return source_scalar_type_by_name(name)
    end

    local function source_native_params_from_meta(params, env)
        local out = {}
        for i = 1, #(params or {}) do
            local param_meta = params[i]
            out[i] = {
                name = param_meta.name,
                t = param_meta.ty_ref and source_native_type_from_meta(param_meta.ty_ref, env)
                    or source_scalar_type_by_name(param_meta.type),
            }
        end
        return out
    end

    local function source_make_native_function(meta, source, env)
        local params = source_native_params_from_meta(meta.params, env)
        ...
        return setmetatable({
            ...
            __source_code = source,
        }, FunctionMT)
    end

    local function source_native_split_path(name)
        local out = {}
        for seg in tostring(name):gmatch("[^.]+") do out[#out + 1] = seg end
        return out
    end

    local function source_native_lookup_type_path(name, env)
        local segs = source_native_split_path(name)
        local scope_types = env and env.types or types
        local value = scope_types[segs[1]] or types[segs[1]]
        assert(value ~= nil, ("unknown Moonlift native type path '%s'"):format(tostring(name)))
        for i = 2, #segs do
            value = value[segs[i]]
            assert(value ~= nil, ("unknown Moonlift native type path '%s'"):format(tostring(name)))
        end
    ...
    source_native_type_from_meta = function(meta, env)
        if meta.tag == "scalar" then
            return source_scalar_type_by_name(meta.name)
        elseif meta.tag == "pointer" then
            return make_ptr(source_native_type_from_meta(meta.inner, env))
        elseif meta.tag == "array" then
            return make_array(source_native_type_from_meta(meta.elem, env), tonumber(meta.len))
        elseif meta.tag == "slice" then
            return make_slice(source_native_type_from_meta(meta.elem, env))
        elseif meta.tag == "func" then
            local params = {}
            for i = 1, #(meta.params or {}) do
                params[i] = { name = tostring(i), t = source_native_type_from_meta(meta.params[i], env) }
            end
            local result_t = meta.result ~= nil and source_native_type_from_meta(meta.result, env) or types.void
            return make_source_func_type(params, result_t)
        elseif meta.tag == "path" then
            return source_native_lookup_type_path(meta.name, env)
        end
        error("unsupported native Moonlift type metadata tag: " .. tostring(meta.tag), 2)
    end

    local function source_make_native_const_value(meta, expected_t, env)
        if meta.tag == "bool" then
            if expected_t ~= nil and expected_t.family ~= "bool" then
                return expected_t(meta.value and 1 or 0)
            end
            return types.bool(meta.value)
        elseif meta.tag == "int" or meta.tag == "float" then
            assert(expected_t ~= nil, "moonlift native constant scalar is missing an expected type")
            return expected_t(meta.value)
        elseif meta.tag == "aggregate" then
            assert(expected_t ~= nil and is_layout_type(expected_t), "moonlift native aggregate constant is missing a layout type")
            local values = {}
            if expected_t.layout_kind == "array" then
                for i = 1, #(meta.fields or {}) do
                    values[i] = source_make_native_const_value(meta.fields[i].value, expected_t.elem, env)
                end
            else
                local ordered = expected_t.fields or {}
                local next_pos = 1
                for i = 1, #(meta.fields or {}) do
                    local field_meta = meta.fields[i]
                    if field_meta.name ~= nil then
                        local field = assert(expected_t.field_lookup and expected_t.field_lookup[field_meta.name],
                            ("unknown native constant field '%s' on %s"):format(field_meta.name, tostring(expected_t.name)))
                        values[field_meta.name] = source_make_native_const_value(field_meta.value, field.type, env)
                    else
                        local field = ordered[next_pos]
                        assert(field ~= nil, "too many positional native aggregate constant fields")
                        values[next_pos] = source_make_native_const_value(field_meta.value, field.type, env)
                        next_pos = next_pos + 1
                    end
                end
            end
            return expected_t(values)
        end
        error("unsupported native Moonlift const metadata tag: " .. tostring(meta.tag), 2)
    end

    local function source_register_native_type(meta, env)
        local T
        if meta.tag == "struct" then
            local fields = {}
            for i = 1, #(meta.fields or {}) do
                fields[i] = { meta.fields[i].name, source_native_type_from_meta(meta.fields[i].ty, env) }
            end
            T = make_struct(meta.name, fields)
        elseif meta.tag == "union" then
            local fields = {}
            for i = 1, #(meta.fields or {}) do
                fields[i] = { meta.fields[i].name, source_native_type_from_meta(meta.fields[i].ty, env) }
            end
            T = make_union(meta.name, fields)
        elseif meta.tag == "tagged_union" then
            local variants = {}
            for i = 1, #(meta.variants or {}) do
                local variant = meta.variants[i]
                local fields = {}
                for j = 1, #(variant.fields or {}) do
                    fields[j] = { variant.fields[j].name, source_native_type_from_meta(variant.fields[j].ty, env) }
                end
                variants[variant.name] = fields
            end
            local spec = { variants = variants }
            if meta.base ~= nil then spec.base = source_native_type_from_meta(meta.base, env) end
            T = make_tagged_union(meta.name, spec)
        elseif meta.tag == "enum" then
            local items = {}
            for i = 1, #(meta.members or {}) do
                items[meta.members[i].name] = meta.members[i].value
            end
            T = make_enum(meta.name, source_native_type_from_meta(meta.base, env), items)
        elseif meta.tag == "slice" then
            T = make_slice(source_native_type_from_meta(meta.elem, env))
        elseif meta.tag == "type_alias" then
            T = source_native_type_from_meta(meta.ty, env)
        else
            error("unsupported native Moonlift module type metadata tag: " .. tostring(meta.tag), 2)
        end
        env.types[meta.name] = T
        env.exports[meta.name] = T
        return T
    end

    local function source_make_native_extern(meta, env)
        return {
            kind = "direct",
            name = meta.name,
            label = meta.name,
            symbol = meta.symbol,
            params = source_native_params_from_meta(meta.params, env),
            result = source_scalar_type_by_name(meta.result),
        }
    end
    ...
    local function source_make_native_module(meta, source)
        ...
        for i = 1, #(meta.types or {}) do
            source_register_native_type(meta.types[i], env)
        end
        for i = 1, #(meta.externs or {}) do
            local info = source_make_native_extern(meta.externs[i], env)
            env.extern_infos[info.name] = info
            env.exports[info.name] = info
        end
        for i = 1, #(meta.consts or {}) do
            local cmeta = meta.consts[i]
            local cty = source_native_type_from_meta(cmeta.ty, env)
            local value = source_make_native_const_value(cmeta.value, cty, env)
            env.values[cmeta.name] = value
            env.exports[cmeta.name] = value
        end

        local funcs = {}
        for i = 1, #(meta.funcs or {}) do
            local fn = source_make_native_function(meta.funcs[i], source, env)
            funcs[i] = fn
            env.values[fn.name] = fn
            env.exports[fn.name] = fn
            if meta.funcs[i].owner ~= nil and meta.funcs[i].method ~= nil then
                local target_t = env.types[meta.funcs[i].owner]
                if target_t ~= nil and rawget(target_t, "method_lookup") ~= nil then
                    rawget(target_t, "method_lookup")[meta.funcs[i].method] = fn
                end
            end
        end
    ...
    local function source_needs_host_env(source)
        return type(source) == "string" and (source:find("@{", 1, true) ~= nil or source:find("?", 1, true) ~= nil)
    end

    local function load_native_code_meta(source, host_env)
        assert(type(backend.source_meta_code) == "function", "moonlift rust source frontend is unavailable: backend.source_meta_code is missing")
        if host_env ~= nil then
            local out, err = backend.source_meta_code(source, host_env)
            assert(out ~= nil, err)
            return out
        end
        ...
        assert(out ~= nil, err)
        native_code_meta_cache[source] = out
        return out
    end

    local function load_native_module_meta(source, host_env)
        assert(type(backend.source_meta_module) == "function", "moonlift rust source frontend is unavailable: backend.source_meta_module is missing")
        if host_env ~= nil then
            local out, err = backend.source_meta_module(source, host_env)
            assert(out ~= nil, err)
            return out
        end
        ...
        assert(out ~= nil, err)
        native_module_meta_cache[source] = out
        return out
    end

    local function load_native_type_meta(source, host_env)
        assert(type(backend.source_meta_type) == "function", "moonlift rust source frontend is unavailable: backend.source_meta_type is missing")
        if host_env ~= nil then
            local out, err = backend.source_meta_type(source, host_env)
            assert(out ~= nil, err)
            return out
        end
        local meta = native_type_meta_cache[source]
        if meta ~= nil then return meta end
        local out, err = backend.source_meta_type(source)
        assert(out ~= nil, err)
        native_type_meta_cache[source] = out
        return out
    end

    local function load_native_extern_meta(source, host_env)
        assert(type(backend.source_meta_extern) == "function", "moonlift rust source frontend is unavailable: backend.source_meta_extern is missing")
        if host_env ~= nil then
            local out, err = backend.source_meta_extern(source, host_env)
            assert(out ~= nil, err)
            return out
        end
        local meta = native_extern_meta_cache[source]
        if meta ~= nil then return meta end
        local out, err = backend.source_meta_extern(source)
        assert(out ~= nil, err)
        native_extern_meta_cache[source] = out
        return out
    end

    local function native_source_code(source, host_env)
        return source_make_native_function(load_native_code_meta(source, host_env), source, nil)
    end

    local function native_source_module(source, host_env)
        return source_make_native_module(load_native_module_meta(source, host_env), source)
    end
    ...
    make_source_func_type = function(params, result_t)
        local parts = {}
    ...
    local function source_lower_stmt_void(stmt, env, in_loop, void_function, return_state)
        ...
        elseif stmt.tag == "for" then
            ...
            local loop_t = source_value_type(start_v) or types.i32
            local finish_e = source_coerce_value(source_lower_expr(stmt.finish, env), loop_t)
            local step_e = stmt.step and source_coerce_value(source_lower_expr(stmt.step, env), loop_t) or as_expr(1, loop_t)
            for_range_(source_coerce_value(start_v, loop_t), finish_e, step_e, function(loop_ref)
    ...
    local function source_terminal_stmt_can_lower_direct_value(stmt)
        return stmt.tag == "return" or stmt.tag == "expr" or stmt.tag == "if" or stmt.tag == "switch"
    end

    local function source_block_can_lower_direct_value(block_ast)
        local stmts = block_ast.stmts or {}
        local last_i = #stmts
        if last_i == 0 then return false end
        for i = 1, last_i - 1 do
            if source_stmt_has_return(stmts[i]) then return false end
        end
        return source_terminal_stmt_can_lower_direct_value(stmts[last_i])
    end
    ...
    local function source_lower_function_body(func_ast, body_env, result_t)
        ...
        if is_void_type(result_t) then
            if not source_block_contains_return(func_ast.body) then
                source_lower_block_void(func_ast.body, body_env, false, true)
                return nil
            end
        ...
        if source_block_can_lower_direct_value(func_ast.body) then
            return source_coerce_value(source_lower_block_value(func_ast.body, body_env, false), result_t)
        end
    ...
    source_lower_expr = function(ast, env)
        ...
        elseif tag == "cast" then
            local to_t = source_resolve_type(ast.ty, env)
            local ok, const_value = pcall(source_eval_const_expr, ast.value, env)
            if ok then return as_expr(source_const_cast_value(to_t, const_value), to_t) end
            return cast(to_t, source_lower_expr(ast.value, env))
        elseif tag == "trunc" then
            local to_t = source_resolve_type(ast.ty, env)
            local ok, const_value = pcall(source_eval_const_expr, ast.value, env)
            if ok then return as_expr(source_const_cast_value(to_t, const_value), to_t) end
            return trunc(to_t, source_lower_expr(ast.value, env))
        elseif tag == "zext" then
            local to_t = source_resolve_type(ast.ty, env)
            local ok, const_value = pcall(source_eval_const_expr, ast.value, env)
            if ok then return as_expr(source_const_cast_value(to_t, const_value), to_t) end
            return zext(to_t, source_lower_expr(ast.value, env))
        elseif tag == "sext" then
            local to_t = source_resolve_type(ast.ty, env)
            local ok, const_value = pcall(source_eval_const_expr, ast.value, env)
            if ok then return as_expr(source_const_cast_value(to_t, const_value), to_t) end
            return sext(to_t, source_lower_expr(ast.value, env))
        elseif tag == "bitcast" then
            local to_t = source_resolve_type(ast.ty, env)
            local ok, const_value = pcall(source_eval_const_expr, ast.value, env)
            if ok then return as_expr(source_const_cast_value(to_t, const_value), to_t) end
            return bitcast(to_t, source_lower_expr(ast.value, env))
        elseif tag == "sizeof" then
        ...
        elseif tag == "if" then
            local function build_branch(i)
                ...
                return if_(
                    source_lower_expr(branch.cond, env),
                    source_lower_branch_value(branch.value, source_child_env(env)),
                    build_branch(i + 1)
                )
            end
            return build_branch(1)
    ...
    local function source_code(source, host_env)
        local env = source_needs_host_env(source) and (host_env or caller_env(2)) or nil
        local out = native_source_code(source, env)
        if env ~= nil then out.__source_host_env = env end
        return out
    end

    local function source_module(source, host_env)
        local env = source_needs_host_env(source) and (host_env or caller_env(2)) or nil
        local out = native_source_module(source, env)
        if env ~= nil then out.__source_host_env = env end
        return out
    end

    local function source_expr(source, host_env)
        assert(type(backend.source_meta_expr) == "function", "moonlift rust source frontend is unavailable: backend.source_meta_expr is missing")
        local env = source_needs_host_env(source) and (host_env or caller_env(2)) or nil
        local meta, err = backend.source_meta_expr(source, env)
        assert(meta ~= nil, err)
        local out = expr(meta.node, source_expr_result_type_by_name(meta.type))
        out.__source_expr = source
        if env ~= nil then out.__source_host_env = env end
        return out
    end
    ...
    local function source_type(source, host_env)
        local env = source_needs_host_env(source) and (host_env or caller_env(2)) or nil
        return source_native_type_from_meta(load_native_type_meta(source, env), source_new_env(host_env or caller_env(2)))
    end
    ...
    local function source_extern(source, host_env)
        local caller = host_env or caller_env(2)
        local env = source_new_env(caller)
        local metas = load_native_extern_meta(source, source_needs_host_env(source) and caller or nil)
        local out = {}
        for i = 1, #(metas or {}) do
            out[i] = source_make_native_extern(metas[i], env)
            out[out[i].name] = out[i]
        end
        if #out == 1 then return out[1] end
        return out
    end

    source_quote_helpers = require("moonlift.source_quotes") {
        parse_backend_ast = parse_backend_ast,
        attach_ast_meta = attach_ast_meta,
        clone_ast = clone_ast,
        caller_env = caller_env,
        source_new_env = source_new_env,
        source_child_env = source_child_env,
        source_merge_host_bindings = source_merge_host_bindings,
        source_resolve_type = source_resolve_type,
        source_infer_expr_type = source_infer_expr_type,
        source_infer_value_type = source_infer_value_type,
        source_prune_type = source_prune_type,
        source_type_name = source_type_name,
        source_lower_expr = source_lower_expr,
        source_emit_side_effect_expr = source_emit_side_effect_expr,
        source_lower_block_void = source_lower_block_void,
        source_lower_block_value = source_lower_block_value,
        source_block_can_lower_direct_value = source_block_can_lower_direct_value,
        source_lower_params = source_lower_params,
        source_infer_function_result = source_infer_function_result,
        source_require_resolved_type = source_require_resolved_type,
        source_bind_local = source_bind_local,
        source_lower_function_body = source_lower_function_body,
        unwrap_source_item = unwrap_source_item,
        source_lower_module_ast = source_lower_module_ast,
        source_type = source_type,
        source_module = source_module,
        quote_expr_capture = quote_expr_capture,
        quote_stmt_capture = quote_stmt_capture,
        quote_block_capture = quote_block_capture,
        is_void_type = is_void_type,
        is_ir_quote = is_ir_quote,
        backend_ast_expr = backend.ast_expr,
        backend_ast_code = backend.ast_code,
        backend_ast_type = backend.ast_type,
        backend_ast_module = backend.ast_module,
    }

    local quote_api = source_quote_helpers.quote_api

    local function no_lua_lowering_api(name)
        error(("moonlift rust-only source mode removed ml.lower.%s; source text must lower in Rust, not via Lua AST lowering"):format(name), 2)
    end

    local lower_api = {
        code = function(ast, host_env)
            return no_lua_lowering_api("code")
        end,
        module = function(ast, host_env)
            return no_lua_lowering_api("module")
        end,
        expr = function(ast, host_env)
            return no_lua_lowering_api("expr")
        end,
        type = function(ast, host_env)
            return no_lua_lowering_api("type")
        end,
        extern = function(ast, host_env)
            return no_lua_lowering_api("extern")
        end,
    }
