...
local scroll_hit_node
scroll_hit_node = function(node, outer, scroll_id, ctx)
    if not node then
        return nil
    end
    ...
    elseif kind == "Key" or kind == "HitBox" or kind == "Pressable" or kind == "Focusable" then
        local frame = resolve_local_frame(node, outer, measure_opts)
        return scroll_hit_node(node.child, frame, scroll_id, ctx)
    elseif kind == "Flex" then
        local placed = flex_layout_children(node, outer, measure_opts)
        for i = #placed, 1, -1 do
            local result = scroll_hit_node(placed[i].node, placed[i].frame, scroll_id, ctx)
            if result ~= nil then
                return result
            end
        end
    ...
    elseif kind == "Pad" then
        ...
        local iw = math_max(0, frame.w - node.insets.l - node.insets.r)
        local ih = math_max(0, frame.h - node.insets.t - node.insets.b)
        return scroll_hit_node(node.child, mkframe(frame.x + node.insets.l, frame.y + node.insets.t, iw, ih), scroll_id, ctx)
    elseif kind == "Stack" then
        local frame = resolve_local_frame(node, outer, measure_opts)
        for i = #node.children, 1, -1 do
            local result = scroll_hit_node(node.children[i], frame, scroll_id, ctx)
            if result ~= nil then
                return result
            end
        end
    ...
    elseif kind == "Clip" then
        local frame = resolve_local_frame(node, outer, measure_opts)
        push_clip(ctx, frame.x + ctx.tx, frame.y + ctx.ty, frame.w, frame.h)
        local result = scroll_hit_node(node.child, frame, scroll_id, ctx)
        pop_clip(ctx)
        return result
    elseif kind == "Transform" then
        local frame = resolve_local_frame(node, outer, measure_opts)
        push_transform(ctx, node.tx, node.ty)
        local result = scroll_hit_node(node.child, mkframe(frame.x, frame.y, frame.w, frame.h), scroll_id, ctx)
        pop_transform(ctx)
        return result
    elseif kind == "Sized" then
        local frame = resolve_local_frame(node, outer, measure_opts)
    ...
    elseif kind == "ScrollArea" then
        ...
        local child_frame, viewport_x, viewport_y, viewport_w, viewport_h = scroll_child_frame(node, frame, measure_opts)
        push_clip(ctx, viewport_x + ctx.tx, viewport_y + ctx.ty, viewport_w, viewport_h)
        local result = scroll_hit_node(node.child, child_frame, next_scroll, ctx)
        pop_clip(ctx)
        if result ~= nil then
            return result
        end
    ...
    elseif kind == "Panel" then
        local frame = resolve_local_frame(node, outer, measure_opts)
        local inset_h, inset_v = style_insets(node.style)
        return scroll_hit_node(node.child, mkframe(frame.x + inset_h, frame.y + inset_v, math_max(0, frame.w - inset_h * 2), math_max(0, frame.h - inset_v * 2)), scroll_id, ctx)
    elseif kind == "Rect" or kind == "Text" or kind == "CustomPaint" or kind == "Spacer" then
        if scroll_id ~= nil then
            local frame = resolve_local_frame(node, outer, measure_opts)
            if frame_hit(ctx, frame) then
                return scroll_id
            end
        ...
        return nil
    elseif kind == "Overlay" then
        return scroll_hit_node(node.base, outer, scroll_id, ctx)
    end
...
function hit.scroll_id(node, frame, x, y, opts)
    local ctx = new_ctx(frame, x, y, opts)
    return scroll_hit_node(node, frame, nil, ctx)
end
