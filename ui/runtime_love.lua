...
local function cursor_name(cursor)
    if cursor == nil or cursor == Style.CursorDefault then return "arrow" end
    if cursor == Style.CursorPointer then return "hand" end
    if cursor == Style.CursorText then return "ibeam" end
    if cursor == Style.CursorMove then return "sizeall" end
    if cursor == Style.CursorGrab then return "hand" end
    if cursor == Style.CursorGrabbing then return "hand" end
    if cursor == Style.CursorNotAllowed then return "no" end
    return "arrow"
end

local function cursor_from_string(name)
    if name == nil or name == "default" then return "arrow" end
    if name == "pointer" then return "hand" end
    if name == "text" then return "ibeam" end
    if name == "move" then return "sizeall" end
    if name == "grab" then return "hand" end
    if name == "grabbing" then return "hand" end
    if name == "not-allowed" then return "no" end
    return "arrow"
end

local function mesh_mode_name(mode)
    if mode == Paint.MeshStrip then return "strip" end
    if mode == Paint.MeshFan then return "fan" end
    return "triangles"
end
...
function M.new(opts)
    ...
    function self:draw_rect(x, y, w, h, visual)
        ...
        if visual.bg ~= 0 then
            love.graphics.setColor(rgba8_to_love(visual.bg, (visual.opacity or 100) / 100))
            love.graphics.rectangle("fill", x, y, w, h, visual.radius or 0, visual.radius or 0)
        end

        if visual.border_w > 0 and visual.border_color ~= 0 then
            local old = love.graphics.getLineWidth()
            love.graphics.setLineWidth(math.max(1, round(visual.border_w)))
            love.graphics.setColor(rgba8_to_love(visual.border_color, (visual.opacity or 100) / 100))
            love.graphics.rectangle("line", x, y, w, h, visual.radius or 0, visual.radius or 0)
    ...
    function self:draw_paint(x, y, w, h, paint)
        ...
        for i = 1, #items do
            ...
            elseif cls == Paint.Arc then
                ...
                love.graphics.setLineWidth(math.max(1, round(item.stroke.width)))
                love.graphics.setColor(rgba8_to_love(item.stroke.rgba8, 1))
                love.graphics.arc("line", "open", round(item.cx), round(item.cy), math.max(0, item.r), item.a1, item.a2, math.max(3, round(item.segments)))
                love.graphics.setLineWidth(old)
