...
do
    ...
    MT.WhenPointer = getmetatable(T.DS.WhenPointer(POINTER_IDLE))
    MT.WhenFocus = getmetatable(T.DS.WhenFocus(FOCUS_BLURRED))
    MT.RequireFlags = getmetatable(T.DS.RequireFlags({}))
    MT.ColorLit = getmetatable(CL)
    MT.SpaceLit = getmetatable(SL)
    MT.ScaleLit = getmetatable(SC)
    MT.FontLit = getmetatable(FL)
    MT.SetBg = getmetatable(T.DS.SetBg(CL))
    MT.SetFg = getmetatable(T.DS.SetFg(CL))
    MT.SetBorder = getmetatable(T.DS.SetBorder(CL))
    MT.SetAccent = getmetatable(T.DS.SetAccent(CL))
    MT.SetRadius = getmetatable(T.DS.SetRadius(SL))
    MT.SetOpacity = getmetatable(T.DS.SetOpacity(SC))
    MT.SetPadH = getmetatable(T.DS.SetPadH(SL))
    MT.SetPadV = getmetatable(T.DS.SetPadV(SL))
    MT.SetGap = getmetatable(T.DS.SetGap(SL))
    MT.SetGapCross = getmetatable(T.DS.SetGapCross(SL))
    MT.SetBorderWidth = getmetatable(T.DS.SetBorderWidth(SL))
    MT.SetFont = getmetatable(T.DS.SetFont(FL))
    MT.SetLineHeight = getmetatable(T.DS.SetLineHeight(LH_AUTO))
    MT.SetTextAlign = getmetatable(T.DS.SetTextAlign(TXT_START))
    MT.SetTextWrap = getmetatable(T.DS.SetTextWrap(TXT_NOWRAP))
    MT.SetOverflow = getmetatable(T.DS.SetOverflow(OV_VISIBLE))
    MT.SetLineLimit = getmetatable(T.DS.SetLineLimit(UNLIMITED_LINES))
end
