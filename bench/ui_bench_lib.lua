...
M.VIEW_W = 1280
M.VIEW_H = 800
M.ENV_CLASS = T.Env.Class(T.Env.Lg, T.Env.Dark, T.Env.MotionSafe, T.Env.D2x)
M.SOLVE_ENV = T.Solve.Env(M.VIEW_W, M.VIEW_H, {})
...
local function build_row(i, changed, serial)
    local tone = (i % 5 == 0) and tw.border_color.sky[500]
        or (i % 3 == 0) and tw.border_color.violet[500]
    ...
    return b.with_input(b.id("row:" .. i), T.Interact.ActivateTarget,
        b.box {
            ...
            b.text { tw.text_sm, tw.fg.slate[300], "row detail text for benchmark item " .. i },
        })
end

function M.workload_flat_list(n, changed, serial)
    local rows = {}
    for i = 1, n do rows[i] = build_row(i, changed, serial) end
    return b.box {
        b.id("root"),
        tw.flow,
        tw.w_px(M.VIEW_W), tw.h_px(M.VIEW_H),
        tw.bg.slate[950],
        tw.p_4,
        tw.gap_y_2,
...
local LOREM = "Compiler-shaped UI benchmark text wrapping through the approximate text backend with enough words to trigger layout work repeatedly."

function M.workload_text_heavy(n, changed, serial)
    local items = {}
    for i = 1, n do
    ...
    return b.box {
        b.id("root"), tw.flow, tw.w_px(M.VIEW_W), tw.h_px(M.VIEW_H), tw.bg.slate[950], tw.p_4, tw.gap_y_3,
        b.text { tw.text_xl, tw.font_semibold, tw.fg.white, "text heavy" },
        b.fragment(items),
    }
end

function M.workload_nested_panels(n, changed, serial)
    local groups = {}
    local per = 6
    for g = 1, n do
        local cards = {}
    ...
    return b.box {
        b.id("root"), tw.flow, tw.w_px(M.VIEW_W), tw.h_px(M.VIEW_H), tw.bg.slate[950], tw.p_4, tw.gap_y_4,
...
M.workloads = {
    flat_list = {
        default_n = 200,
        build = function(n, changed, serial) return M.workload_flat_list(n, changed, serial) end,
        changed_index = function(n) return math.max(1, math.floor(n / 2)) end,
    },
    text_heavy = {
        default_n = 120,
        build = function(n, changed, serial) return M.workload_text_heavy(n, changed, serial) end,
        changed_index = function(n) return math.max(1, math.floor(n / 2)) end,
    },
    nested_panels = {
        default_n = 40,
        build = function(n, changed, serial) return M.workload_nested_panels(n, changed, serial) end,
        changed_index = function(n) return math.max(1, math.floor((n * 6) / 2)) end,
    },
}

function M.build_auth(workload_name, n, changed, serial)
    local w = assert(M.workloads[workload_name], "unknown workload")
    return w.build(n, changed, serial)
end

function M.changed_index(workload_name, n)
    local w = assert(M.workloads[workload_name], "unknown workload")
    return w.changed_index(n)
end

function M.lower_one(auth, theme, env_class)
    return pvm.one(ui.lower.phase(auth, theme or M.THEME, env_class or M.ENV_CLASS))
end

function M.render_ops(layout, solve_env, text_system)
    return pvm.drain(ui.render.root(layout, solve_env or M.SOLVE_ENV, text_system == nil and M.TEXT_SYSTEM or text_system))
end

function M.full_run(auth, opts)
    opts = opts or {}
    local layout = M.lower_one(auth, opts.theme, opts.env_class)
    return ui.runtime.run(opts.driver, {
        pointer_x = opts.pointer_x or -1,
        pointer_y = opts.pointer_y or -1,
        scrolls = opts.scrolls or {},
    }, ui.render.root(layout, opts.solve_env or M.SOLVE_ENV, opts.text_system == nil and M.TEXT_SYSTEM or opts.text_system))
end

function M.report_string()
