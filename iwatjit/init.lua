...
local function cache_lookup_entry(cache, node, ...)
    ...
    if root ~= nil and argc == 0 and root.entry ~= nil then
        return root.entry
    end
...
local function direct_cached_count(plan)
    touch_entry(plan._iwj_cache_entry)
    return plan.count
end

local function direct_cached_sum(plan)
    touch_entry(plan._iwj_cache_entry)
    local cached = rawget(plan, "_iwj_sum")
    if cached ~= nil then
        return cached
    end
    local slab = plan.slab
    local acc = 0
    if slab ~= nil then
        local ptr = slab.ptr
        for i = 0, slab.count - 1 do
            acc = acc + tonumber(ptr[i])
        end
    else
        local values = assert(plan.values, "cached_seq is missing slab and values storage")
        for i = 1, plan.count do
            acc = acc + host_value(values[i])
        end
    end
    plan._iwj_sum = acc
    return acc
...
local function direct_cached_one(plan, default)
    touch_entry(plan._iwj_cache_entry)
    if plan.count <= 0 then
        return default
    end
    local cached = rawget(plan, "_iwj_first")
    if cached ~= nil then
        return cached
    end
    local slab = plan.slab
    if slab ~= nil then
        cached = tonumber(slab.ptr[0])
    else
        local values = assert(plan.values, "cached_seq is missing slab and values storage")
        cached = host_value(values[1])
    end
    plan._iwj_first = cached
    return cached
end

local function direct_cached_drain(plan)
    touch_entry(plan._iwj_cache_entry)
    local slab = plan.slab
    if slab ~= nil then
        return read_slab_seq(slab)
    end
    local values = assert(plan.values, "cached_seq is missing slab and values storage")
    local out = {}
    for i = 1, plan.count do
        out[i] = host_value(values[i])
    end
    return out
end
...
local function evict_entry(rt, entry)
    ...
    if entry_lru_slot(entry) >= 0 and rt.result.lru ~= nil then
        lru_remove(rt, entry)
...
local function commit_entry(entry)
    ...
    cached._iwj_first = count > 0 and entry.values[1] or nil
    do
        local acc = 0
        for i = 1, count do
            acc = acc + entry.values[i]
        end
        cached._iwj_sum = acc
    end
    ...
    if entry.rt.result.lru ~= nil then
...
function M.runtime(opts)
    ...
    return {
        ...
        result = {
            ...
            lru = opts.result_capacity and native_lru_manager("iwj_result", opts.entry_capacity or 262144, engine) or nil,
        },
    }
end
...
function M.count(plan)
    local resolved = resolve_stream(plan)
    if resolved.kind == "cached_seq" then
        return direct_cached_count(resolved)
    end
...
function M.sum(plan, opts)
    opts = opts or {}
    local resolved = resolve_stream(plan)
    if resolved.kind == "cached_seq" and opts.vector_t == nil then
        return direct_cached_sum(resolved)
    end
    if plan_has_recording(resolved) then
...
function M.one(plan, default)
    local resolved = resolve_stream(plan)
    if resolved.kind == "cached_seq" then
        return direct_cached_one(resolved, default or 0)
    end
    if plan_has_recording(resolved) then
...
function M.drain(plan)
    local resolved = resolve_stream(plan)
    if resolved.kind == "cached_seq" then
        return direct_cached_drain(resolved)
    end
    if plan_has_recording(resolved) then
...
local legacy_runtime = M.runtime
local legacy_phase = M.phase

local SpecPhaseMT = {}
local ScalarPhaseMT = {}
local TerminalMT = {}
SpecPhaseMT.__index = SpecPhaseMT
ScalarPhaseMT.__index = ScalarPhaseMT
TerminalMT.__index = TerminalMT
...
local function is_type_like(t)
    return type(t) == "table" and t.size ~= nil and t.name ~= nil
end

local function assert_type_like(t, where)
    assert(is_type_like(t), where)
    return t
end
...
local function normalize_field_entry(k, v, where)
    if type(k) == "number" then
        assert(type(v) == "table", where .. ": array field entry must be a table")
        if v[1] ~= nil then
            return {
                name = assert(v[1], where .. ": field missing name"),
                type = assert_type_like(v[2], where .. ": field missing type"),
                opts = v[3],
            }
        end
        return {
            name = assert(v.name, where .. ": field missing name"),
            type = assert_type_like(v.type, where .. ": field missing type"),
            opts = v.opts or (v.align and { align = v.align } or nil),
        }
    end
    local field_t = v
    local opts = nil
    if type(v) == "table" and v.size == nil and v.type ~= nil then
        field_t = v.type
        opts = v.opts or (v.align and { align = v.align } or nil)
    end
    return {
        name = k,
        type = assert_type_like(field_t, where .. ": field '" .. tostring(k) .. "' must be a watjit type"),
        opts = opts,
    }
end

local function normalize_fields(fields, where)
    if fields == nil then
        return {}
    end
    assert(type(fields) == "table", where .. " must be a table")
    local out = {}
    if is_array_table(fields) then
        for i = 1, #fields do
            out[i] = normalize_field_entry(i, fields[i], where)
        end
        return out
    end
    local keys = {}
    for k, _v in pairs(fields) do
        assert(type(k) == "string", where .. ": map-form fields must use string keys")
        keys[#keys + 1] = k
    end
    table.sort(keys)
    for i = 1, #keys do
        out[i] = normalize_field_entry(keys[i], fields[keys[i]], where)
    end
    return out
end

local function to_struct_fields(fields)
    local out = {}
    for i = 1, #fields do
        local f = fields[i]
        out[i] = { f.name, f.type, f.opts }
    end
    return out
end

local function normalize_cache(cache)
    cache = shallow_copy(cache)
    if cache.args == nil then cache.args = "full" end
    if cache.policy == nil then cache.policy = "bounded_lru" end
    if cache.cost == nil then cache.cost = "bytes" end
    return cache
end

local function normalize_result(result, fallback_item_t, default_storage)
    result = shallow_copy(result)
    if result.storage == nil then result.storage = default_storage or "slab_seq" end
    if result.item_t == nil then result.item_t = fallback_item_t end
    if result.inline_capacity == nil then result.inline_capacity = 0 end
    if result.item_t ~= nil then
        assert_type_like(result.item_t, "result.item_t must be a watjit type")
    end
    return result
end

local function typed_params(params, where)
    params = params or {}
    assert(type(params) == "table", where .. " must be a table")
    local out = {}
    for i = 1, #params do
        local p = params[i]
        assert(type(p) == "table" and p.t ~= nil and p.name ~= nil, where .. " entries must be typed named watjit values")
        out[i] = p
    end
    return out
end

local function status_enum(name)
    return wj.enum(name .. "_Status", wj.u8, {
        Empty = 0,
        Recording = 1,
        Live = 2,
        Shared = 3,
...
local function make_result_layout(name, result, Status)
    if result.storage == "scalar" then
        return wj.struct(name .. "_Result", {
            { "status", Status },
            { "value", assert_type_like(result.item_t, "scalar result.item_t must be a watjit type") },
            { "cost_bytes", wj.i32 },
        }, { packed = false })
    end
    return wj.struct(name .. "_Result", {
...
local function build_machine_layouts(kind, spec)
    local Status = status_enum(spec.name)
    local Param = wj.struct(spec.name .. "_Param", to_struct_fields(spec.param_fields), { packed = false })
    local State = wj.struct(spec.name .. "_State", to_struct_fields(spec.state_fields), { packed = false })
    local Result = make_result_layout(spec.name, spec.result, Status)
    local Entry = wj.struct(spec.name .. "_Entry", {
        { "status", Status },
        { "key", spec.key_t },
        { "arg_tag", wj.u32 },
        { "result", wj.ptr(Result) },
        { "lru_prev", wj.i32 },
        { "lru_next", wj.i32 },
        { "cost_bytes", wj.i32 },
    }, { packed = false })
    local Cursor = wj.struct(spec.name .. "_Cursor", {
        { "status", Status },
        { "param", wj.ptr(Param) },
        { "state", wj.ptr(State) },
        { "entry", wj.ptr(Entry) },
...
local function build_machine_common(kind, spec)
    assert(type(spec) == "table", "iwatjit machine spec must be a table")
    assert(type(spec.name) == "string", "iwatjit machine spec.name must be a string")
    assert_type_like(spec.key_t, "iwatjit machine spec.key_t must be a watjit type")
    local normalized = shallow_copy(spec)
    normalized.param_fields = normalize_fields(spec.param, spec.name .. ".param")
    normalized.state_fields = normalize_fields(spec.state, spec.name .. ".state")
    normalized.cache = normalize_cache(spec.cache)
    normalized.result = normalize_result(spec.result, spec.item_t or spec.value_t, kind == "scalar_phase" and "scalar" or "slab_seq")
    local layouts = build_machine_layouts(kind, normalized)
    local obj = {
        kind = kind,
        name = spec.name,
        spec = normalized,
        key_t = spec.key_t,
        item_t = spec.item_t,
        value_t = spec.value_t,
        cache = normalized.cache,
        result = normalized.result,
        hooks = shallow_copy(spec.hooks),
        gen = spec.gen,
    ...
    if kind == "phase" then
        return setmetatable(obj, SpecPhaseMT)
    elseif kind == "scalar_phase" then
        return setmetatable(obj, ScalarPhaseMT)
    end
    return setmetatable(obj, TerminalMT)
end

function SpecPhaseMT:describe()
    return {
        kind = self.kind,
        name = self.name,
        key_t = self.key_t.name,
        item_t = self.item_t and self.item_t.name or nil,
        cache = shallow_copy(self.cache),
        result = shallow_copy(self.result),
        param_fields = self.spec.param_fields,
        state_fields = self.spec.state_fields,
        layouts = { Param = self.Param, State = self.State, Result = self.Result, Entry = self.Entry, Cursor = self.Cursor },
    }
end

function SpecPhaseMT:gen_fn(opts)
    opts = opts or {}
    local extra = typed_params(opts.params, "phase:gen_fn opts.params")
    local self_phase = self
    local params = { wj.i32 "param_base", wj.i32 "state_base" }
    for i = 1, #extra do params[#params + 1] = extra[i] end
    return wj.fn {
        name = opts.name or (self.name .. "_gen"),
        params = params,
        ret = opts.ret or wj.i32,
        body = function(param_base, state_base, ...)
            local P = self_phase.Param.at(param_base)
            local S = self_phase.State.at(state_base)
            return opts.body(P, S, ...)
        end,
    }
end

function ScalarPhaseMT:describe()
    return {
        kind = self.kind,
        name = self.name,
        key_t = self.key_t.name,
        value_t = self.value_t and self.value_t.name or nil,
        cache = shallow_copy(self.cache),
        result = shallow_copy(self.result),
        param_fields = self.spec.param_fields,
        state_fields = self.spec.state_fields,
...
function ScalarPhaseMT:compute_fn(opts)
    opts = opts or {}
    local extra = typed_params(opts.params, "scalar_phase:compute_fn opts.params")
    local self_phase = self
    local params = { wj.i32 "param_base" }
    for i = 1, #extra do params[#params + 1] = extra[i] end
    return wj.fn {
        name = opts.name or (self.name .. "_compute"),
        params = params,
        ret = opts.ret or self.value_t or wj.i32,
        body = function(param_base, ...)
            local P = self_phase.Param.at(param_base)
            return opts.body(P, ...)
        end,
    }
...
function TerminalMT:describe()
    return {
        ...
        result = shallow_copy(self.result),
        state_fields = self.spec.state_fields,
        layouts = { State = self.State, Result = self.Result, Entry = self.Entry, Cursor = self.Cursor },
    }
end

function TerminalMT:step_fn(opts)
    opts = opts or {}
    local extra = typed_params(opts.params, "terminal:step_fn opts.params")
    local self_terminal = self
    local params = { wj.i32 "state_base" }
    for i = 1, #extra do params[#params + 1] = extra[i] end
    return wj.fn {
        name = opts.name or (self.name .. "_step"),
        params = params,
        ret = opts.ret or wj.i32,
        body = function(state_base, ...)
            local S = self_terminal.State.at(state_base)
            return opts.body(S, ...)
        end,
    }
end
...
function M.runtime(opts)
    opts = opts or {}
    local memory = shallow_copy(opts.memory)
    local tables = shallow_copy(opts.tables)
    local legacy_opts = shallow_copy(opts)
    if memory.result_bytes ~= nil then legacy_opts.result_capacity = memory.result_bytes end
    if memory.temp_bytes ~= nil then legacy_opts.temp_capacity = memory.temp_bytes end
    if tables.entry_capacity ~= nil then legacy_opts.entry_capacity = tables.entry_capacity end
    local rt = legacy_runtime(legacy_opts)
    rt.memory = {
        result_bytes = memory.result_bytes or legacy_opts.result_capacity,
        temp_bytes = memory.temp_bytes or legacy_opts.temp_capacity,
    }
    rt.tables = {
        entry_capacity = tables.entry_capacity or legacy_opts.entry_capacity or 262144,
    }
    return rt
end

function M.phase(a, b, c, d)
    if b == nil and c == nil and d == nil and type(a) == "table" and a.name ~= nil and a.gen ~= nil then
        assert_type_like(a.item_t, "phase spec.item_t must be a watjit type")
        return build_machine_common("phase", a)
    end
    return legacy_phase(a, b, c, d)
end

function M.scalar_phase(spec)
    assert(type(spec.compute) == "function", "scalar_phase spec.compute must be a function")
    assert_type_like(spec.value_t, "scalar_phase spec.value_t must be a watjit type")
    return build_machine_common("scalar_phase", spec)
end

function M.terminal(spec)
    assert(type(spec.step) == "function", "terminal spec.step must be a function")
    assert_type_like(spec.item_t, "terminal spec.item_t must be a watjit type")
    local t = shallow_copy(spec)
    t.key_t = t.key_t or wj.i32
    return build_machine_common("terminal", t)
end

M.CONTINUE = wj.iter.CONTINUE
M.BREAK = wj.iter.BREAK
M.HALT = wj.iter.HALT
M.CONTINUE_VALUE = wj.iter.CONTINUE_VALUE
M.BREAK_VALUE = wj.iter.BREAK_VALUE
M.HALT_VALUE = wj.iter.HALT_VALUE
