...
local fn = fir.make_simd(coeffs, wj.simd.f64x2)
...
for i = 0, n - 1 do
    assert(math.abs(y[i] - expect[i]) < 1e-12, ("simd fir mismatch at %d: got=%f expect=%f"):format(i, y[i], expect[i]))
end

print("watjit: fir simd ok")
