-- bench_pvm_json_raw.lua — pvm2 vs pvm3 on JSON lowering to raw tokens
--
-- Domain:
--   Json.Value tree  ->  flat raw token stream
--
-- This benchmark removes JsonCmd object construction entirely. Tokens are plain Lua
-- values (mostly cached strings), so the benchmark isolates lowering/traversal and
-- cache reuse without ASDL checked-constructor churn in the output layer.
...
local TOK_END_ARR   = "]"
local TOK_TRUE      = "true"
local TOK_FALSE     = "false"
local TOK_NULL      = "null"

local key_tok_cache = {}
local str_tok_cache = setmetatable({}, { __mode = "k" })
local num_tok_cache = setmetatable({}, { __mode = "k" })

local function TOK_KEY(key)
    local tok = key_tok_cache[key]
    if tok == nil then
        tok = "K:" .. key
        key_tok_cache[key] = tok
    end
    return tok
end

local function TOK_STR_NODE(node)
    local tok = str_tok_cache[node]
    if tok == nil then
        tok = "S:" .. node.value
        str_tok_cache[node] = tok
    end
    return tok
end

local function TOK_NUM_NODE(node)
    local tok = num_tok_cache[node]
    if tok == nil then
        tok = "N:" .. tostring(node.value)
        num_tok_cache[node] = tok
    end
    return tok
end
...
local function emit_handwritten(root)
    ...
    local function emit(node)
        ...
        if kind == "Str" then
            n = n + 1; out[n] = TOK_STR_NODE(node)
        elseif kind == "Num" then
            n = n + 1; out[n] = TOK_NUM_NODE(node)
        elseif kind == "Bool" then
            n = n + 1; out[n] = node.value and TOK_TRUE or TOK_FALSE
        elseif kind == "Null" then
            n = n + 1; out[n] = TOK_NULL
        elseif kind == "Arr" then
            n = n + 1; out[n] = TOK_BEGIN_ARR
            local items = node.items
            for i = 1, #items do emit(items[i]) end
            n = n + 1; out[n] = TOK_END_ARR
        elseif kind == "Obj" then
            n = n + 1; out[n] = TOK_BEGIN_OBJ
            local pairs = node.pairs
            for i = 1, #pairs do
                local pair = pairs[i]
                n = n + 1; out[n] = TOK_KEY(pair.key)
                emit(pair.value)
            end
            n = n + 1; out[n] = TOK_END_OBJ
        elseif kind == "Pair" then
            n = n + 1; out[n] = TOK_KEY(node.key)
            emit(node.value)
        else
...
local emit2_trip
emit2_trip = pvm2.verb_memo("emit2_json_raw_trip", {
    [J.Str] = function(node)
        return T.unit(TOK_STR_NODE(node))
    end,
    [J.Num] = function(node)
        return T.unit(TOK_NUM_NODE(node))
    end,
    [J.Bool] = function(node)
        return T.unit(node.value and TOK_TRUE or TOK_FALSE)
    end,
    [J.Null] = function()
        return T.unit(TOK_NULL)
    end,
    [J.Pair] = function(node)
        return pvm2.concat_all({
            { T.unit(TOK_KEY(node.key)) },
            { emit2_trip(node.value) },
        })
    end,
    [J.Arr] = function(node)
        local parts = { { T.unit(TOK_BEGIN_ARR) } }
        for i = 1, #node.items do
            parts[#parts + 1] = { emit2_trip(node.items[i]) }
        end
        parts[#parts + 1] = { T.unit(TOK_END_ARR) }
        return pvm2.concat_all(parts)
    end,
    [J.Obj] = function(node)
        local parts = { { T.unit(TOK_BEGIN_OBJ) } }
        for i = 1, #node.pairs do
            parts[#parts + 1] = { emit2_trip(node.pairs[i]) }
        end
        parts[#parts + 1] = { T.unit(TOK_END_OBJ) }
        return pvm2.concat_all(parts)
    end,
})

local emit2_flat
emit2_flat = pvm2.verb_flat("emit2_json_raw_flat", {
    [J.Str] = function(node, out)
        out[#out + 1] = TOK_STR_NODE(node)
    end,
    [J.Num] = function(node, out)
        out[#out + 1] = TOK_NUM_NODE(node)
    end,
    [J.Bool] = function(node, out)
        out[#out + 1] = node.value and TOK_TRUE or TOK_FALSE
    end,
    [J.Null] = function(_, out)
        out[#out + 1] = TOK_NULL
    end,
    [J.Pair] = function(node, out)
        out[#out + 1] = TOK_KEY(node.key)
        emit2_flat(node.value, out)
    end,
    [J.Arr] = function(node, out)
        out[#out + 1] = TOK_BEGIN_ARR
        local items = node.items
        for i = 1, #items do emit2_flat(items[i], out) end
        out[#out + 1] = TOK_END_ARR
    end,
    [J.Obj] = function(node, out)
        out[#out + 1] = TOK_BEGIN_OBJ
        local pairs = node.pairs
        for i = 1, #pairs do emit2_flat(pairs[i], out) end
        out[#out + 1] = TOK_END_OBJ
    end,
})

local compile2 = pvm2.lower("compile_json_raw2", function(root)
    local out = {}
    emit2_flat(root, out)
    return out
end)
...
local emit3
emit3 = pvm3.phase("emit3_json_raw", {
    [J.Str] = function(node)
        return pvm3.once(TOK_STR_NODE(node))
    end,
    [J.Num] = function(node)
        return pvm3.once(TOK_NUM_NODE(node))
    end,
    [J.Bool] = function(node)
        return pvm3.once(node.value and TOK_TRUE or TOK_FALSE)
    end,
    [J.Null] = function()
        return pvm3.once(TOK_NULL)
    end,
    [J.Pair] = function(node)
        local g1, p1, c1 = pvm3.once(TOK_KEY(node.key))
        local g2, p2, c2 = emit3(node.value)
        return pvm3.concat2(g1, p1, c1, g2, p2, c2)
    end,
    [J.Arr] = function(node)
        local g1, p1, c1 = pvm3.once(TOK_BEGIN_ARR)
        local g2, p2, c2 = pvm3.children(emit3, node.items)
        local g3, p3, c3 = pvm3.once(TOK_END_ARR)
        return pvm3.concat3(g1, p1, c1, g2, p2, c2, g3, p3, c3)
    end,
    [J.Obj] = function(node)
        local g1, p1, c1 = pvm3.once(TOK_BEGIN_OBJ)
        local g2, p2, c2 = pvm3.children(emit3, node.pairs)
        local g3, p3, c3 = pvm3.once(TOK_END_OBJ)
        return pvm3.concat3(g1, p1, c1, g2, p2, c2, g3, p3, c3)
    end,
})

local compile3 = pvm3.lower("compile_json_raw3", function(root)
...
for _, count in ipairs({ 20, 100, 300 }) do
    ...
    print(string.format("\n═══ RAW TOKENS items=%d  tokens=%d  iters=%d ═══", count, #hand, N))
...
print("\n═══ RAW TOKEN cache quality snapshot (items=100, edit one item name) ═══")
