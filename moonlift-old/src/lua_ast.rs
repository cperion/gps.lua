...
fn push_loop_var_init(out: &mut String, var: &LoopVarInit) {
    out.push_str("{ name = ");
    push_quoted(out, &var.name);
    out.push_str(", ty = ");
    push_type(out, &var.ty);
    out.push_str(", init = ");
    push_expr(out, &var.init);
    out.push_str(" }");
}

fn push_loop_next_assign(out: &mut String, next: &LoopNextAssign) {
    out.push_str("{ name = ");
    push_quoted(out, &next.name);
    out.push_str(", value = ");
    push_expr(out, &next.value);
    out.push_str(" }");
}

fn push_loop_head(out: &mut String, head: &LoopHead) {
    match head {
        LoopHead::While { vars, cond, .. } => {
            out.push_str("{ tag = \"while\", vars = ");
            push_list(out, vars, push_loop_var_init);
            out.push_str(", cond = ");
            push_expr(out, cond);
            out.push_str(" }");
        }
        LoopHead::Over {
            ...
            out.push_str("{ tag = \"over\", index = ");
            push_quoted(out, index);
            out.push_str(", domain = ");
            push_expr(out, domain);
            out.push_str(", carries = ");
            push_list(out, carries, push_loop_var_init);
            out.push_str(" }");
        }
    }
}
...
fn push_stmt(out: &mut String, stmt: &Stmt) {
    match stmt {
        ...
        Stmt::Loop(v) => {
            out.push_str("{ tag = \"loop\", head = ");
            push_loop_head(out, &v.head);
            out.push_str(", body = ");
            push_block(out, &v.body);
            out.push_str(", next = ");
            push_list(out, &v.next, push_loop_next_assign);
            out.push_str(" }");
        }
        Stmt::Switch(v) => {
...
fn push_expr(out: &mut String, expr: &Expr) {
    match &expr.kind {
        ...
        ExprKind::Loop(v) => {
            out.push_str("{ tag = \"loop_expr\", head = ");
            push_loop_head(out, &v.head);
            out.push_str(", body = ");
            push_block(out, &v.body);
            out.push_str(", next = ");
